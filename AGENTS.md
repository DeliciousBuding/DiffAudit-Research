...
- Active work: `Trajectory Generation Privacy artifact gate completed after Model Will Tell / DRC. arXiv 2605.15246 / Privacy Evaluation of Generative Models for Trajectory Generation is cross-domain trajectory/mobility privacy evidence, not a current image or latent-image asset. It evaluates LSTM-TrajGAN, MoveSim, DiffTraj, and Diff-RNTraj; the two diffusion trajectory results are near random, with DiffTraj AUC-ROC = 0.5012 and Diff-RNTraj AUC-ROC = 0.4949, while the only clearly positive table result is GAN MoveSim AUC-ROC = 0.7002. The checked public surface is paper-source-only: the 106,622 byte arXiv source tarball has only 00README.json, IEEEtran.bst, IEEEtran.cls, main.tex, and ref.bib; exact-title, arXiv-id, author/title, and trajectory-generation GitHub repository/code searches returned no official repository, code, score, or artifact hits. Decision: cross-domain trajectory paper-source-only watch / diffusion trajectory MIA weak / no code-score artifact / no download / no GPU release / no admitted Platform or Runtime row. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after trajectory generation privacy artifact gate.`
