...
- Active work: `Progress review completed after CPSample, DSiRe / LoRA-WiSE, hyperparameter-free SecMI, DME, FreMIA, and CopyMark gates. Status: latest verdict note, workspace-evidence index, Research ROADMAP, AGENTS, defense/intake/implementation workspace notes, and root ROADMAP are synchronized to the CPSample defense artifact gate. CPSample remains defense watch-plus only: official OpenReview code supplement and small inference_attack loss fragments exist, but immutable denoiser/classifier checkpoint hashes, exact subset-index manifests, row-bound protected/unprotected score packets, ROC arrays, AUC/ASR/TPR-at-FPR metrics, retained-utility metric contract, and ready verifier are missing. DSiRe / LoRA-WiSE remains a future weight-only dataset-size recovery lane candidate, not per-sample MIA. No CIFAR-10/CelebA/LSUN/Stable Diffusion/LoRA-WiSE/model/checkpoint/generated-image download, python main.py or dsire.py execution, CPU sidecar, GPU work, Platform/Runtime row, schema change, or product copy is released. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after CPSample defense artifact gate.`
