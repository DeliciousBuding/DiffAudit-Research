...
- Active work: `Silent Brush / Art Arena artifact gate completed after Trajectory Generation Privacy. arXiv 2605.17500 / The Silent Brush: Evaluating Artistic Style Leakage in AI Art Generation is text-to-image diffusion-adjacent privacy evidence, but its claim boundary is style leakage / copyright evaluation rather than the current per-sample membership contract. The metadata-readable anonymous resource surface exposes code/notebook inventory only: ArtArena.ipynb, README.md, ET/MD eval and infer scripts, FT_models.py, get_leadger.py, prep scripts, CSD/model.py, CSD/utils.py, and figure PDFs. arXiv source HEAD reports a 35,598,493 byte gzip and was not downloaded. Exact-title repository search, arXiv-id repository search, and ArtArena-EBE4 code search returned no official public GitHub artifact hits; exact-title code search only found paper-index aggregator entries. Decision: style-leakage semantic-shift watch / anonymous code-notebook inventory only / no row-bound membership artifact / no download / no GPU release / no admitted Platform or Runtime row. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after Silent Brush / Art Arena artifact gate.`
