...
- Active work: `Eidetic lightweight triage sync completed after the ReDiffuse raw transfer provenance recheck. hackerman70000/eidetic is a small code-only Carlini-style extraction/MIA toolkit at commit 1aa786bd6e6c39af1da1ac3581e1835f9b55b2ee. Its README reports CIFAR-10 Strong-LiRA metrics, but the repo commits no shadow checkpoints, immutable split manifest, per-row scores, ROC arrays, metric JSON, generated response packet, or ready verifier; experiments download CIFAR-10 and require local shadow checkpoints. Decision: lightweight GitHub false-positive / code-only toolkit / no artifact packet / no download / no GPU release / no admitted Platform or Runtime row. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after GitHub lightweight diffusion MIA triage.`
