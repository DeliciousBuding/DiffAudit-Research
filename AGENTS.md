...
- Active work: `DCR copying manifest gate completed after the FCRE medical frequency artifact gate. Status: DCR is copying/memorization semantic-shift watch-plus / code-and-caption-manifest-public / split-link-unavailable / no MIA score packet / no download / no GPU release / no admitted row; FCRE remains paper-source-only / medical-image cross-domain watch / no official code / no split or score packet / no download / no GPU release / no admitted row; Tabular Privacy Leakage TDM remains single-table tabular diffusion watch-plus / official code-public / no paper score packet / no download / no GPU release; TMIA-DM remains paper-only temporal-noise gradient MIA / reported metrics only / no code-score artifact / no download / no GPU release / no admitted row. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after DCR copying manifest gate, with LAION payloads, DCR Drive split folders, Stable Diffusion weights, generated image sets, retrieval outputs, FeTS, ChestX-ray8, CIFAR-10, medical-image payloads, Berka, Diabetes, MIDST Google Drive resources, ClavaDDPM checkpoints, generated synthetic tables, target/shadow training, DDIM reconstruction, frequency-band sweeps, DCR retrieval, Tartan Federer, Ensemble, and EPT execution still blocked.`
