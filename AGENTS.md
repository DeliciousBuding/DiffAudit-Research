...
- Active work: `Discrete DLM withdrawn artifact gate completed after the Eidetic lightweight triage sync. arXiv 2605.16445 / Membership Inference Attacks on Discrete Diffusion Language Models is withdrawn in the current arXiv record, has no current PDF, and says citations/co-authors need verification before a future version. Its abstract reports strong MDLM membership metrics, but GitHub repository searches for the exact title and topic-style query returned no repositories, code search for 2605.16445 returned no hits, and the public surface has no official code, hashable target checkpoint, immutable member/nonmember manifest, score rows, ROC arrays, metric JSON, generated response packet, or ready verifier. Decision: withdrawn arXiv / DLM paper-source-only / no official code / no artifact packet / no download / no GPU release / no admitted Platform or Runtime row. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after Discrete DLM withdrawn artifact gate.`
