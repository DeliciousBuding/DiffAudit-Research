...
- Active work: `Hyperparameter-free SecMI reproduction gate completed after DME, FreMIA, and CopyMark gates. Status: mohammadKazzazi/Membership-Inference-Attack-against-Diffusion-Models is a third-party SecMI-family code/report surface with claimed CIFAR-100 seed-0 improvements over baseline SecMINNs, but no reusable score rows, ROC arrays, metric JSON, trained attacker weights, immutable new split manifest, independent target checkpoint, or no-training verifier; no CIFAR/SecMI checkpoint download, attacker training, CPU sidecar, GPU work, Platform/Runtime row, or product copy is released. DME remains a non-duplicate complexity-bias diffusion-MIA watch item, but the official repo is README-only with no linked paper, implementation code, immutable split manifest, target checkpoint, generated sample, score row, ROC array, metric JSON, figure artifact, or verifier. FreMIA remains an ICML 2026 direct diffusion-model MIA watch item with arXiv source tables/figures and a stub official GitHub repo, but no code, splits, checkpoints, score rows, machine-readable ROC arrays, metric JSON, or verifier. CopyMark remains official score-artifact support evidence with public member/nonmember image logs, selected all-step score tensors, GSA feature/XGBoost files, and laion_ridar/mixing results, but no checkpoint hashes, compact row-ID-bound score manifest, small immutable data/checkpoint packet, or ready verifier output. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after hyperparameter-free SecMI reproduction gate.`
