...
- Active work: `DSiRe / LoRA-WiSE dataset-size boundary gate completed after hyperparameter-free SecMI, DME, FreMIA, and CopyMark gates. Status: MoSalama98/DSiRe plus the public non-gated MoSalama98/LoRA-WiSE HF dataset is a strong future weight-only privacy lane candidate with official code, 7 dataset configs, 2,050 LoRA fine-tuned model rows, 101 parquet shards, and reported dataset-size recovery MAE = 0.36 images, but its claim is aggregate LoRA fine-tuning dataset-size recovery rather than per-sample membership inference; it has no member/nonmember query rows, generated response packets, per-sample score rows, ROC arrays, AUC/ASR/TPR-at-FPR metrics, or current Platform/Runtime consumer semantics. No LoRA-WiSE dataset/image/Stable Diffusion/LoRA tensor download, dsire.py run, CPU sidecar, GPU work, Platform/Runtime row, or product copy is released. Hyperparameter-free SecMI remains third-party SecMI-family support evidence with code/report but no reusable score packet or verifier. DME and FreMIA remain artifact-incomplete watch items. CopyMark remains official score-artifact support evidence but lacks checkpoint hashes, compact row-ID-bound score manifest, small immutable data/checkpoint packet, and ready verifier output. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after DSiRe / LoRA-WiSE dataset-size boundary gate.`
