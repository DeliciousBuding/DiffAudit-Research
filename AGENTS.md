...
- Active work: `ReDiffuse SD bundle-root normalization is the latest engineering fix. probe-rediffuse-sd-assets now accepts both the imported parent bundle and the raw/ child directory for collaborator-stable-diffusion-rediffuse-20260516, so the existing 5000-row 2500/2500 Stable Diffusion ReDiffuse candidate packet no longer false-blocks when pointed at the transfer parent. Latest candidate verdict remains Identity-Focused Inference / Extraction: arXiv 2410.10177 is paper-source-only identity-level privacy evidence with no official code, checkpoint-bound identity/row/score/ROC/metric/verifier artifacts, no download, no GPU release, and no admitted Platform or Runtime row. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after ReDiffuse SD bundle-root normalization.`
