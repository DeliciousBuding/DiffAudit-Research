...
- Active work: `Roadmap current-state cleanup and entrypoint sync after PR #267 / #268. The latest repository operating-system update deduplicated the old CopyMark-era current-state tables and clarified that the 2026-05-13 Leader Steering block is historical anti-stationery guidance; the Current Long-Horizon State table is the active three-slot source of truth. The latest candidate verdict remains the Discrete DLM withdrawn artifact gate: arXiv 2605.16445 / Membership Inference Attacks on Discrete Diffusion Language Models is withdrawn in the current arXiv record, has no current PDF, and has no official code, hashable target checkpoint, immutable member/nonmember manifest, score rows, ROC arrays, metric JSON, generated response packet, or ready verifier. Decision: withdrawn arXiv / DLM paper-source-only / no official code / no artifact packet / no download / no GPU release / no admitted Platform or Runtime row. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after Discrete DLM withdrawn artifact gate. No candidate check, download, CPU/GPU sidecar, Platform row, or Runtime schema changed in the roadmap cleanup.`
