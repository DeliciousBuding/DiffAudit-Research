...
- 2026-06-30 current override: active work is Paper 1 Phase G H1/DAAB
  ...
  `<DOWNLOAD_ROOT>/checkpoints/ddpm-cifar10-seed43/checkpoint-step624000.pt`;
  do not resume from heartbeat-only step 625700. The default CUDA environment
  is `retrace-tr`, not `ai-learning`. Same-trajectory DDPM-800k N=512 is
  documented as AUC 0.576 but still lacks archived raw JSON; fix that evidence
  gap before final paper use. Do not reopen H2 sweeps, C14 expansion, scnet,
  Beans/Fashion/MIDST repeats, or defense-transfer claims unless a new user
