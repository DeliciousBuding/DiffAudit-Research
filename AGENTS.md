...
- Active work: `CPSample defense artifact gate completed after DSiRe / LoRA-WiSE, hyperparameter-free SecMI, DME, FreMIA, and CopyMark gates. Status: OpenReview LIBLIlk5M9 / CPSample has an ICLR 2025 official code supplement with diffusion/classifier code and small inference_attack score-text fragments, but it ships no immutable denoiser/classifier checkpoint hashes, exact subset-index manifests, row-bound protected/unprotected score packet, ROC arrays, AUC/ASR/TPR-at-FPR metrics, retained-utility metric contract, or ready verifier. It remains defense watch-plus only: no CIFAR-10/CelebA/LSUN/Stable Diffusion/model/checkpoint/generated-image download, python main.py execution, inference_attack run, CPU sidecar, GPU work, Platform/Runtime row, or product copy is released. DSiRe / LoRA-WiSE remains a future weight-only dataset-size recovery lane candidate, not per-sample MIA. CopyMark remains official score-artifact support evidence but lacks checkpoint hashes, compact row-ID-bound score manifest, small immutable data/checkpoint packet, and ready verifier output. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after CPSample defense artifact gate.`
