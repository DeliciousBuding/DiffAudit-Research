...
- Active work: `Model Will Tell / DRC artifact gate completed after the roadmap current-state cleanup. arXiv 2403.08487 / Model Will Tell: Training Membership Inference for Diffusion Models proposes Degrade Restore Compare, a restoration-prior membership observable that intentionally degrades an image, restores it with the target diffusion model, and compares semantic similarity against the original. The checked public surface is paper-source-only: the 1,745,140 byte arXiv source tarball has 19 TeX/style/bibliography/figure entries only; the paper says the authors intend to release code but gives no official code URL; exact-title, DRC phrase, author-name, and arXiv-id GitHub repository/code searches returned no official code or artifact hits. Decision: paper-source-only DRC restoration MIA watch / no official code / no split-score artifact / no download / no GPU release / no admitted Platform or Runtime row. active_gpu_question = none; next_gpu_candidate = none; CPU sidecar = none selected after Model Will Tell DRC artifact gate.`
