...
class ReviewReDiffuseCheckpointPortabilityGateTests(unittest.TestCase):
    ...
    def test_missing_checkpoint_returns_structured_blocked_summary(self) -> None:
        repo_root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory(dir=repo_root) as tmpdir:
            root = Path(tmpdir)
            comparison = root / "800k.pt"
            ...
            completed = subprocess.run(
                [
                    ...
                    "scripts/review_rediffuse_checkpoint_portability_gate.py",
                    "--collaborator-checkpoint",
                    (root / "missing.pt").relative_to(repo_root).as_posix(),
                    "--comparison-checkpoint",
                    comparison.relative_to(repo_root).as_posix(),
                    "--split-path",
                    split.relative_to(repo_root).as_posix(),
            ...
            self.assertTrue(output.is_file())
            payload = json.loads(output.read_text(encoding="utf-8"))
            self.assertFalse(payload["checkpoints"]["collaborator_750k"]["exists"])
            self.assertFalse(payload["release_gate"]["architecture_compatible"])
