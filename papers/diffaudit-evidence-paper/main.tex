...
evidence bundle, DiffAudit admits five cross-permission rows spanning black-box
reconstruction risk, gray-box PIA, a stochastic-dropout comparator, white-box
GSA, and a DPDM defense comparator. We also report a controlled H2
...
contract. Finally, bounded scouts and a frozen metadata-only search batch over
GitHub and arXiv show why second-asset claims require artifact contracts rather
...
standalone reproducibility paper would require an independent metadata-only
expansion before making prevalence claims. The protocol is deliberately
lightweight: freeze the search date, sources, and keywords; record public
metadata only; label each artifact with the six gates; and separate public
artifacts from internal controls and collaborator-local packets. No large model
or dataset download is justified until the metadata gate shows that a row-bound
claim could change.
...
DiffAudit route decisions. The 2026-05-26 fixed-search batch and selected-corpus
consistency pass reduce project-history and labeling bias, but they remain a
small metadata-only process trace rather than a prevalence estimate. A
standalone reproducibility paper would need a larger frozen corpus or a second
