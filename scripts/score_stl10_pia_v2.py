"""PIA v2 — paper-accurate attack from ReDiffuse micro_attack.py.
Uses DDIM trajectory: epsilon prediction at t=0, then denoising consistency check.
"""
import sys, os
import numpy as np
import torch
...
_OUTPUT = os.environ.get('DIFFAUDIT_OUTPUT', os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'outputs', 'stl10-10k'))
...
BATCH = 32  # PIA attack needs more VRAM per sample
T = 1000
...
def main():
    ...
    # PIA attack setup — positional call matches micro_attack.py pattern
    # (PIA.__init__ and Attacker.__init__ have different param names; positional
    #  matching routes EpsGetter into the correct super() slot)
    betas = torch.linspace(1e-4, 0.02, T).to(DEVICE)
    eps_getter = EpsGetter(model)
    attacker = components.PIA(
        betas,       # → PIA.betas → super().betas
        200,         # → PIA.interval → super().interval
        1,           # → PIA.attack_num → super().attack_num
        100,         # → PIA.eps_getter → super().k (placeholder, not used by PIA)
        eps_getter,  # → PIA.normalize → super().eps_getter ✓ (the actual getter!)
        10,          # → PIA.denormalize → super().average (placeholder)
        None,        # → PIA.lp (none; we compute distance manually)
    )

    splits = np.load(os.path.join(_SUPP, 'DDPM', 'STL10_train_ratio0.5.npz'))
    ...
    def run_attack(loader, desc):
        ...
        with torch.no_grad():
            for i, batch in enumerate(loader):
                ...
                # L2 distance between epsilon predictions
    ...
    print(f"\n=== STL-10 PIA v2 Results (paper-accurate) ===", flush=True)
