"""CIFAR-10 DDPM training — same architecture as STL-10, optimized for RTX 4070.
CIFAR-10 is 32×32 native, 25k members, should converge faster than STL-10.
"""
...
OUTPUT_DIR = os.environ.get('DIFFAUDIT_OUTPUT', os.path.join(_RESEARCH, 'outputs', 'cifar10-10k'))
PT_PATH = os.path.join(DATA_ROOT, 'shared', 'datasets', 'cifar10_member_25k.pt')

sys.path.insert(0, os.path.join(SUPP, 'DDPM'))
from diffusion import GaussianDiffusionTrainer, GaussianDiffusionSampler
from model_unet import UNet

BATCH, TOTAL_STEPS = 64, 10000  # CIFAR-10 is 32×32 native, can fit larger batch
IMG_SIZE, T = 32, 1000
EMA_EVERY = 10
...
def main():
    ...
    for step in range(1, TOTAL_STEPS + 1):
        ...
        if step % 5000 == 0:
            torch.save({'model': ema_model.state_dict(), 'step': step, 'losses': losses},
