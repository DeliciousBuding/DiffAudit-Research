...
```json
{
  ...
  "target_member_dataset": "<DATASET_ROOT>/target_member.pt",
  "target_nonmember_dataset": "<DATASET_ROOT>/target_non_member.pt",
  "shadow_member_dataset": "<DATASET_ROOT>/shadow_member.pt",
  "shadow_nonmember_dataset": "<DATASET_ROOT>/shadow_non_member.pt",
  "target_model_dir": "<CHECKPOINT_ROOT>/target_checkpoint",
  "shadow_model_dir": "<CHECKPOINT_ROOT>/shadow_checkpoint",
  "repo_root": "<DIFFAUDIT_ROOT>/Research/external/Reconstruction-based-Attack",
...
这个接口的用途是让 Platform 或 local integrations 在刷新页面后重新发现已提交任务，而不是只能依赖内存中的 `job_id`。
...
6. 调 `GET /api/v1/audit/jobs` 恢复当前 workspace 已有任务列表
