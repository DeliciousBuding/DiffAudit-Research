...
## 2026-06-06 CCF-A 研究推进路线

最新决策：Research 不再把当前稿件状态解释为单纯 publish/polish。Direction A
仍是唯一 LaTeX 主稿，但现有证据更接近 CCF-B-style measurement potential；
若要支撑 CCF-A，需要新增一个独立科学证据对象，而不是继续给弱路线补工程。
内部路线图见
[`docs/internal/ccf-a-research-roadmap-2026-06-06.md`](docs/internal/ccf-a-research-roadmap-2026-06-06.md)。
当前 E2 v2 internal pilot 已通过 Go/No-Go，输出在
[`docs/internal/e2-pilot-2026-06-06-v2/`](docs/internal/e2-pilot-2026-06-06-v2/)；
后续进入
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/`](docs/internal/e2-n50-freeze-preflight-2026-06-06/)。
这些只用于内部校准和 freeze preflight，不作为论文结果或 external
adjudication evidence，也不升级 Direction A。N50 preflight 的严格 triage
当前是：`0` 行可直接进入 external denominator；v1 row review 已覆盖 `5`
个既有 external candidates 和 `11` 个 scout gate-review rows，并把 immediate
package-work 收窄到 `1` 行：`E2Q-005` Tracing the Roots。`E2Q-004` CLiD、
`E2Q-006` CopyMark 和 `E2Q-011` Quantile/SecMI 只能保留为 bounded support，
`E2Q-009` MIDST 和多数 scout rows 只能作为 tabular/cross-modal/defense/
copying/backup 支撑，不能并入 image-diffusion external denominator。另有 `27` 个
no-download public-surface scout candidates 用于补齐 corpus freeze。E1
second-asset scout 已收敛到
[`docs/internal/e1-manifest-first-scout-2026-06-06.md`](docs/internal/e1-manifest-first-scout-2026-06-06.md)，
其中 2026-06-08 新增 NDSS-324 / Zenodo `10.5281/zenodo.13371475` 为
compute-gated 候选：公开包有 fine-tuned SD LoRA checkpoint 和
member/nonmember dataset PKL，但中央目录没有 score vectors/ROC/metric/verifier，
所以还不是 second public score/response asset。同日 bounded artifact probe
已验证完整 `736,366,195` byte ZIP，MD5 匹配 Zenodo，`zipfile.testzip()` 通过；
静态 schema probe 只看到 `image`/`text` 字段，没有 `id`、`file_name` 或
`image_id` 这类 row-bound identifier，且 split 仍是 `shadow_member_proxy`
级别的 local semantics。没有 row-bound manifest 前不得占用 GPU。
2026-06-09 晚间对同一 high-value watchlist 做了字段级 no-download refresh；
本轮 live refresh snapshot 为 `2026-06-09T11:40:53.841513+00:00`。
`9 / 9` public identities 仍匹配，admission-grade compact reopen surface 为
`0`。NDSS-324 的 GitHub `main` 仍为
`93ee8dd4d12697354cd182461a9aa268b8de63e6`，Zenodo record `13371475` 仍只有同一个
`736,366,195` byte ZIP；CopyMark 和 MoFit 源身份也未变化。MoFit 保持最强
bounded support follow-up，但仍缺 explicit row IDs、immutable checkpoint
identity、official metric JSON/ROC/verifier 和 surface-delta controls。当前
freeze handoff 入口是
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_public_source_freeze_ledger_2026_06_09.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_public_source_freeze_ledger_2026_06_09.md)：
`11` rows，`bounded_support=1`，`packet_ready_only=1`，
`no_current_upgrade=9`，compute-release rows 为 `0`。当前 blocker 仍是
independent external labeling 或新的 row-bound public score/response surface。

2026-06-09 support-surface 复核新增四条 route closure：`E2SCT-031`
SAMA/DLM、`E2SCT-032` MIA-EPT、`E2SCT-033` Diffusion MIA 和 `E2SCT-034`
ReMIA。SAMA 是 DLM/text public-code surface；MIA-EPT 是 tabular
result-page surface；Diffusion MIA 是 image-diffusion code-and-split surface；
ReMIA 是 tabular aggregate-result archive，公开 `2,879` 个 JSON 但没有
row-scale score/label arrays。四者都只作为 C5 source-provenance sidecar 和
route-closure hygiene，不进入 C14、不进入 N50 external denominator、不作为
admitted evidence、不占用 second public score/response asset slot，也不释放
GPU/DCU。当前 blocker 仍是 independent external labeling 或新的 row-bound
public score/response surface。

2026-06-06 续推进：`E2Q-005` 的 OpenReview supplement 已完成公开身份复核。
ZIP size `45,499,156`、SHA-256
`62e9ae3833bcc0f102612d05898262eea2b6025fe8949a72c3f055a8534c7b41`、中央目录
`12` 项、四个 CIFAR10 feature tensor hash、torch CPU shape check 和既有 replay JSON
均对齐；单行 blind-review skeleton 已建立在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2q005_single_row_blind_review_skeleton_2026_06_06.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2q005_single_row_blind_review_skeleton_2026_06_06.md)。
这只把 `E2Q-005` 推进为 feature-packet-only single-row package-work，不改变
`0` external denominator 结论，也不释放 GPU/DCU。

同日 public-source challenger scout 继续确认：没有当前候选能超过 `E2Q-005`。
`E2Q-006` CopyMark 是最近的 image-diffusion challenger，但仍缺 compact public
row manifest；`E2Q-004` CLiD 缺 row-to-COCO identity manifest；`E2Q-011`
Quantile/SecMI 是 third-party SecMI-style support；`E2SCT-001` MT-MIA 是最好的
tabular/relational 另开分母线索，但不能并入当前 image-diffusion denominator。
结论记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_row_bound_challenger_scout_2026_06_06.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_row_bound_challenger_scout_2026_06_06.md)。
本轮后续 route decision 已记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_next_route_decision_2026_06_06.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_next_route_decision_2026_06_06.md)：
CLiD 仍是 identity-missing bounded support；MT-MIA 不在当前 image-diffusion
N50 周期打开 tabular/relational stratum；当前 CCF-A 升级路线转向 larger
corpus + external audit + false-promotion baselines。
Measurement-route 的执行缺口板见
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_measurement_route_gap_board_2026_06_06.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_measurement_route_gap_board_2026_06_06.md)，
下一轮只做 no-download public-surface checks 和 false-promotion baseline
tagging，不释放 GPU/DCU。

2026-06-07 续推进：gap board 的前四个 no-download checks 已完成。
`E2SCT-004` GenAI Confessions / STROLL 暴露 `100` 对
`intraining` / `outoftraining` annotation rows，但没有公开 row-bound
generated outputs、DreamSim scores、metric JSON 或 verifier；`E2SCT-012`
Shake-to-Leak 暴露 runnable code，但 private set、checkpoints、responses、
SecMI scores 和 extraction candidates 都是 runtime products；`E2SCT-016`
MIAHOLD/HOLD++ 暴露 defense code、split hints、CIFAR HOLD config 和 PIA-style
metric code，但没有 checkpoint-bound scores、ROC arrays、metric JSON/CSV、
generated responses 或 verifier；`E2SCT-021` ELSA Health Privacy 暴露 starter
MIA code、example labels/predictions 和 metric contract，但实际 challenge
targets、labels、predictions、Noisy Diffusion datasets、metadata 和 participant
artifacts 仍是 platform-gated 或 submission-bound。四者都只进入
false-promotion exemplar pool，
不进入 external denominator，不作为 admitted evidence，也不释放 GPU/DCU。四行已汇总为
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_false_promotion_exemplar_summary_2026_06_07.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_false_promotion_exemplar_summary_2026_06_07.md)，
作为后续 weak-rule baseline 设计对象，而不是 external adjudication evidence。
同日补完第二批 semantic / consumer-boundary checks：`E2SCT-002` DMin 是
attribution-vs-membership false-promotion exemplar，`E2SCT-005` DIFFENCE 是
classifier-defense consumer-boundary false-promotion exemplar，`E2SCT-013`
DCR 是 copying-vs-membership false-promotion exemplar，`E2SCT-009`
Memorization Anisotropy 是 prompt-memorization false-promotion exemplar，
`E2SCT-014` CDI 是 dataset-level-vs-per-sample false-promotion exemplar。九行都只进入
false-promotion exemplar pool，不进入 external denominator，不作为 admitted
evidence，也不释放 GPU/DCU。
同轮 post-C14 扩展复核又把 `E2SCT-011` VAE2Diffusion 加入 baseline：
当前 repo head 为 `d530fbb7e0aca488e63637167b4d64539397bcf7`，README 仍保留空的
split/checkpoint 下载链接，非 vendored project tree 中没有 split/checkpoint/
score/ROC/metric/verifier payload，脚本仍依赖本地 split、checkpoint、pullback/per-dim
cache 和 CUDA 执行。它是 code-and-empty-asset-link false-promotion exemplar，
不是 admitted evidence、external denominator 或 compute release。C14 因此扩展为
`10` 行。
随后 `E2SCT-020` LSA-Probe 行级复核完成：当前项目 README 仍说明 full
implementation 和 reproducibility instructions 要等 paper acceptance 后发布；
GitHub releases 为 `0`；GitHub Pages demo 的 `adversarial_costs.json`、
`roc_curves.json` 和 `main_results.json` 仍可读，但同目录
`generate_demo_data.py` 明确写明生成 mock demo data，使用 `np.random.seed(42)`、
每个 timestep `500 / 500` member/nonmember gamma draw，并生成 ROC/main-results
JSON。因此它是 mock-demo-score false-promotion exemplar，不是 admitted
evidence、external denominator、真实 score packet 或 compute release。C14 因此扩展为
`11` 行。
随后 `E2SCT-019` VidLeaks T2V 行级复核完成：Zenodo record/API 仍公开，
DOI 为 `10.5281/zenodo.17972831`，文件目录暴露 `wangli-codesT2V_MIA-v1.0.1.zip`
代码快照，关联 GitHub tag 当前为 `404`；既有本地 archive verdict 也记录只有
README/script/ROC PNG surface，没有生成视频、feature CSV、score rows、exact
video member/nonmember manifest、target checkpoint 或 verifier packet。因此它是
t2v-code-snapshot false-promotion exemplar，不是 admitted evidence、external
denominator、text-to-video 产品支持或 compute release。C14 因此扩展为 `12` 行。
这些 baseline 已同步进入 Direction A 主稿：`claim_trace.csv` C14 后续已更新为
thirteen-row false-promotion baseline，
`data/false_promotion_exemplars.csv`、`data/false_promotion_rule_summary.csv`
和 `figures/false_promotion_exemplars.pdf` 由
`papers/diffaudit-evidence-paper/scripts/build_paper_assets.py` 生成，
同脚本还生成 `data/false_promotion_external_review_packet.csv` 和
`data/false_promotion_external_review_template.csv`，并把
`versions/direction-a-false-promotion-audit-codebook.md` 纳入 C14
provenance。三者只是 external-style review 准备材料，不是已完成 external
adjudication 或 reviewer reliability evidence。
`main.tex` 只允许写成 weak-rule / semantic-boundary false-promotion baseline，
不写成 admitted evidence、N50 denominator、field prevalence 或 compute
release。
同轮最新 no-download metadata refresh（`2026-06-07T14:15:27.640409+00:00`）
把 scout gate queue 收敛为 `7` 行；排除已经进入 C14 的 `E2SCT-002`、
`E2SCT-004`、`E2SCT-011`、`E2SCT-020` 和 `E2SCT-019` 后，`E2SCT-022` Tabular Privacy
Leakage TDM 又完成首轮 public-surface check，并关闭为 support-only /
tabular-lane watch-plus：官方 MIDST toolkit 会触发 weak-rule pressure，但当前
公开面没有 paper-bound Berka/Diabetes replay packet、target identity、exact
split manifest、score rows、ROC arrays、metric JSON 或 no-training verifier。
它不进入 C14，也不释放 compute。`E2SCT-023` FERMI 也完成首轮 public-surface
check，并关闭为 paper-source-only / reported-metric support：arXiv paper 和
HTML surface 可读，但没有官方公开 code、target/split packet、score rows、ROC
arrays、metric JSON 或 verifier。它不进入 C14，也不释放 compute。下一批
post-C14 false-promotion corpus 扩展候选收敛为 `0` 行。
队列记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_false_promotion_expansion_queue_2026_06_07.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_false_promotion_expansion_queue_2026_06_07.md)。
队列当前只是空队列状态记录；不得把 C14 从 `13` 行扩大，不得写成
admitted/denominator evidence，也不得释放 GPU/DCU，直到新的专门 row-level check
产出 clean exemplar 并重新生成论文资产和 review bundle。

2026-06-08 续推进：Direction A 主稿完成一轮 C14 论文硬化和 release-packet
闭环。摘要、contribution 和 C14 小节已移除 `12/12` 这类 denominator-shaped
主文表述，改为 `thirteen selected no-download rows` / `selected stress rows`
口径；C14 仍是 false-promotion baseline stress object，不是 prevalence、
external adjudication、reviewer reliability、admitted evidence 或 compute
release。新增的
`data/false_promotion_author_gate_matrix.csv`、
`data/false_promotion_gate_summary.csv` 和
`figures/false_promotion_gate_matrix.pdf` 只作为 author-keyed post-label
comparison 资产进入 anonymous supplement 和 maintainer-only post-label key ZIP，
不放入主文图表，避免把作者标签呈现成外部审查结果。`check_e2_freeze_preflight.py`
现在直接校验 gate matrix / gate summary 的 13 行、7x4 outcome grid、row ID、
gate values、`label_source=author_key_not_external_label`、`compute_release=no`
和 no-denominator/no-reliability 边界；`export_paper_supplement.py`、
`export_false_promotion_review_bundle.py`、`check_paper_release_packet.py` 与
`run_pr_checks.py` 也新增 output freshness 检查，防止新增资产后 ZIP/manifest
仍停在旧版本。该中间版本产生 10 页 letter PDF 和 50-file supplement；后续
hard-blind reviewer / post-label key 双 ZIP 及当前可发布 hash 已在下文覆盖；
`run_pr_checks.py`
通过。

同日 post-C14 scout 复查 `E2SCT-010` SimA：官方仓库仍是有价值的 score-norm /
denoiser-output mechanism reference，branch 仍为
`97dce4fec6030094fc722557a8b03b6858eab37b`，但 README 的 DDPM 与 SD1.4
split/checkpoint 链接仍为空，SD1.4 checkpoint 仍需邮件获取，脚本与
`diffusers/src/mia/attack.py` 仍依赖本地 `/path/to/...` 和
`/banana/ethan/MIA_data/...` 路径，未发现公开 split manifest、checkpoint hash、
score rows、ROC arrays、metric JSON/CSV 或 verifier。结论记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct010_sima_public_surface_refresh_2026_06_08.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct010_sima_public_surface_refresh_2026_06_08.md)。
SimA 只保留为 support-only code-public mechanism watch-plus，不进入 C14、
不进入 external denominator、不进主文、不释放 GPU/DCU。

同日新增的 `E2SCT-028` SD-MIA / `wanghl21/SD-MIA` 也完成首轮
no-download public-surface check。CVF 2026 paper 和 GitHub branch
`89384223da4ef95f9bddb3d1e222ccf339b914ac` 都是公开且相关的
pre-training T2I black-box MIA surface；但公开树只暴露 README、notebook、
figures、pipeline script 和 source code。实际 `data/original.json`、扰动
JSON、`res/.../attack_results.json`、per-row pooled score table、generated
response packet、ROC/metric JSON、split manifest 和 no-training verifier 都未
提交。结论记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct028_sdmia_public_surface_check_2026_06_08.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct028_sdmia_public_surface_check_2026_06_08.md)。
SD-MIA 只保留为 support-only code-public pre-training T2I MIA reference，
不进入 C14、不进入 external denominator、不作为 external adjudication /
reviewer reliability evidence，也不释放 GPU/DCU。

同日 `E2SCT-003` DurMI TTS 也完成 no-download public-surface check：
OpenReview submission/supplement metadata 和 Zenodo `12` 个数据/检查点文件列表公开，
既有 evidence card 记录了 GradTTS LJSpeech `5,977 / 5,977` 精确 split，但当前
公开面没有 ready duration-loss score arrays、ROC arrays、metric JSON、generated
result graphs 或 no-training verifier，且 DiffAudit 尚未打开 TTS/audio consumer
lane。结论记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct003_durmi_tts_public_surface_check_2026_06_08.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct003_durmi_tts_public_surface_check_2026_06_08.md)。
DurMI 只保留为 support-only TTS/audio cross-modal watch-plus，不进入 C14、
不进入 external denominator、不作为 admitted evidence，也不释放 GPU/DCU。

同日 `E2SCT-025` Hyperparameter-Free SecMI reproduction 也完成 no-download
public-surface check：GitHub branch `3a8855cb54bbff9d15fb19e734b2feadd0cb12bb`
稳定，README 报告 CIFAR-100 seed-0 aggregate AUC/TPR，源码可见并包含
SecMI/download/cache/attacker 运行路径；但公开面没有 per-sample score rows、
ROC arrays、metric JSON、trained attacker weights、generated response packet 或
no-training verifier，且执行需要 SecMI official assets、SharePoint checkpoints、
CIFAR 数据、local caches 和 attacker training。结论记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct025_hyperfree_secmi_public_surface_check_2026_06_08.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct025_hyperfree_secmi_public_surface_check_2026_06_08.md)。
`E2SCT-025` 只保留为 support-only duplicate SecMI-family evidence，不进入
C14、不进入 external denominator、不作为 second independent response/score
asset 或 admitted evidence，也不释放 GPU/DCU。

同日 `E2SCT-024` DME 也完成 no-download public-surface check：GitHub
`HEAD`/`main` 指向 `ae0cc48476746945720bf24b42d4f9dfecb6de31`，没有 tag
refs；GitHub HTML 暴露 `1 Commit`、`No releases published` 和 `README.md`；
raw README 长度 `248`，SHA-256
`b0d1cf04d92d47577a830e6d57477d10d3909ee138ef10b7dea6ed02ecedd225`，只声明
DME 的 official PyTorch implementation。当前公开面没有实现代码、paper
protocol、target/split manifest、checkpoint、score rows、ROC/metric JSON 或
verifier。结论记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct024_dme_public_surface_check_2026_06_08.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct024_dme_public_surface_check_2026_06_08.md)。
`E2SCT-024` 只作为 official-repo-stub false-promotion exemplar 加入 C14，
使 selected stress rows 变为 `13`；它不进入 external denominator，不作为
admitted evidence、external adjudication / reviewer reliability evidence 或
Platform/Runtime evidence，也不释放 GPU/DCU。

同轮继续补强 C14 external-review readiness：false-promotion review bundle
现在是严格的 hard-blind pre-label reviewer packet。reviewer-facing ZIP 只保留
`REVIEWER_README.md`、`REVIEWER_DECLARATION.md`、`REVIEWER_PUBLIC_URLS.csv`、
blocker-blinded packet、blank template、sanitized row trace、codebook 和 launch
protocol；full row trace、cached evidence notes、claim trace、source
provenance、source rows、author gate matrix、author adjudication key、rule
summary 和 summary figures 全部隔离到 separate maintainer-only
`diffaudit-false-promotion-post-label-key.zip` 的 `post-label-author-key/`
下，不得在 labels/declarations final 前分享给 reviewer。`scripts/export_false_promotion_review_bundle.py` 现在生成
reviewer-safe `data/false_promotion_row_trace.csv`，并在 `--check-output`
中校验没有顶层 `evidence-notes/`、没有 author-keyed basename 泄露到
pre-label 路径。codebook / launch protocol / BUILD / README 同步改为这个
隔离边界；主稿只做低风险语言收紧，不改变 C14 证据状态。

后续聚合器仍保持机器可读的保守状态语义：
`scripts/aggregate_false_promotion_external_review.py` 输出 packet-level
`false_promotion_external_review_packet_status.csv`，区分
`packet_label_readiness`、`reviewer_count_status`、`majority_resolution_status`
和 `reliability_threshold_status`，并把
`completed_external_adjudication_allowed`、`reliability_claim_allowed` 和
`compute_release_allowed` 固定为 `0`，直到另有显式协议和论文措辞支持。
synthetic reviewer CSV 只验证聚合器行为，不进入 evidence。该中间版本随后被
paper-method / reviewer-risk pass 覆盖；当前可发布 `paper.pdf` 和 anonymous
supplement hash 见下一段。
false-promotion reviewer ZIP 为 4 reviewer source files + 4 generated reviewer
aid rows / 9 entries，SHA-256
`d91a48276e8e80089507b4c092cf376beaa2c822446abee7b9d252047a3e53db`；
post-label key ZIP 为 38 source files / 39 entries，SHA-256
`6371e0168b674c6511132b92a163f6ca865986380f6f3903c19c29c39b2f0a2d`。
这一步提高的是 label-readiness 和 reviewer-independence hygiene；它仍不产生
external labels、reviewer reliability、N50 denominator、field prevalence 或
compute release。下一步最高价值仍是执行真实 bounded 3-reviewer C14 label
protocol，或继续 targeted no-download 搜索新的 row-bound public artifact
surface；不得用 synthetic label、GPU/DCU sweep 或更多文档包装替代真实证据对象。

同日 paper-method/release hardening 又完成一轮：Direction A 主稿新增
gate-assignment procedure 框，把 `T,S,O,M,C,Delta` 绑定、固定 gate 顺序、
first blocker、narrowest state 和 replay tier 写成可执行规则；删去重复的
claim-boundary 示例表以保持 10 页。`scripts/check_paper_release_packet.py`
现在除 PDF/manifest/provenance 外，还校验 `main.tex`/`refs.bib` 引用一致性、
BibTeX 必要字段、placeholder 文献、reportable 状态的 required gates、非
reportable claim 的 `first_blocker`、`replay_tier` 和 candidate non-admission
wording。H2 主图改为代码生成的 portability-boundary inset：img2img `25/25`
只显示为 no-admission-path boundary，`10/10` 只显示为 diagnostic，不作为
admitted/portable evidence。随后 reviewer-risk wording pass 又把题名从
`protocol evaluation` 收紧为 claim-admission protocol with boundary cases，
并补入 Blind-MIA split-confound 引用：当 member/nonmember 来源可分时，
foundation-model blind baseline 可能压过 MIA attack，所以 split gate 必须
要求 membership semantics，而不是只接受两个方便的 source pools。H2 也改为
metric-strength boundary case，避免被读成独立 admitted attack 或 protocol-wide
performance evaluation。最新 title/wording cleanup 进一步把题名改为
`A Claim-Admission Protocol for Diffusion Membership-Inference Scores: Boundary Cases and Evidence States`，
并把 C14 写成 pre-label stress object 而不是 comparison framework。当前
`paper.pdf` 为 10 页 letter，SHA-256
`E1EDA02E14D3799F825FD0FEEF994466957F4CCB31AA4DC225A99D958A30B103`；
anonymous supplement 为 63 files，SHA-256
`7FF2D4A8AC49BBBA57213B7C4168DB120DDC8CDCD48ED6257EA7F01ADC2E9B25`；
false-promotion reviewer ZIP SHA-256 更新为
`D91A48276E8E80089507B4C092CF376BEAA2C822446ABEE7B9D252047A3E53DB`；
post-label key ZIP SHA-256 为
`6371E0168B674C6511132B92A163F6CA865986380F6F3903C19C29C39B2F0A2D`。
`check_paper_release_packet.py`、`run_pr_checks.py` 和
`export_false_promotion_review_bundle.py --check-output` 均通过。这个进展提高
protocol 可复核性、图表表达和 reviewer-facing framing，但仍不改变
C14/N50/admitted-evidence/compute release 状态。

同轮 C14 reviewer-bundle manifest 又完成一个低风险可核验性修正：
`false_promotion_review_bundle_manifest.csv` 和 ZIP 内
`REVIEW_BUNDLE_MANIFEST.csv` 现在同时列出 4 个 source-backed reviewer files
和 4 个 generated reviewer aids（`REVIEWER_README.md`、
`REVIEWER_DECLARATION.md`、`REVIEWER_PUBLIC_URLS.csv`、sanitized reviewer row
trace）。这只减少 reviewer 侧 ZIP 复核歧义；external labeling 仍未开始，真实
reviewer CSV/declaration 返回前不得报告 external adjudication、reviewer
reliability、N50 denominator、admitted evidence 或 compute release。

随后 reviewer-facing URL hygiene 又补了一步：生成的
`REVIEWER_PUBLIC_URLS.csv` 现在带 `public_url_note` 列，用于标出
`FPR-E2SCT-019` 的 Zenodo 主证据仍可用、相关 GitHub tag 在 no-download
row check 中已返回 `404`。这只是发包导航提示，不改变 source row、C14 label、
admitted/N50/compute 状态或 reviewer 独立性边界。

同轮 aggregator 也补齐 pre-label status-only 输出：在没有 reviewer CSV 时，
`scripts/aggregate_false_promotion_external_review.py` 会写
`build/false_promotion_external_review_packet_status.csv` 和
`build/false_promotion_external_review_aggregation.md`，其中
`packet_label_readiness=prepared_no_reviewer_csvs`、`reviewer_count_status=none`、
`allowed_claim_scope=packet_ready_only`，且
`completed_external_adjudication_allowed`、`reliability_claim_allowed`、
`compute_release_allowed` 均为 `0`。它不会写 majority labels、disagreement、
declaration、agreement 或 author-key comparison 文件；这只是发包前状态锚点，
不是外部审查结果。无标签运行还会清理旧的 label-dependent CSV，避免 pre-label
launch 目录误暴露历史 majority labels。
本轮又把这个状态锚点接入 release gate：`run_pr_checks.py` 现在先刷新
C14 aggregation output，`check_paper_release_packet.py` 随后要求 no-reviewer
status CSV/Markdown 存在，校验 `packet_ready_only` 语义、三个 claim switch
均为 `0`，并在没有 reviewer CSV 时拒绝任何陈旧 label-dependent CSV 或丢失
no-reviewer / not-external-adjudication 边界文本。这只是防止误宣称；不产生
external labels、reviewer reliability、N50 denominator、field prevalence 或
compute release。

同轮继续把 `MoFit` 从历史 code-TBW verdict 更新为当前 public score-surface
candidate：官方 ICLR 2026 repo 已公开 COCO 小文本分数文件，`scripts/replay_mofit_public_coco_scores.py`
可在不 clone、不下载图片/模型、不跑 GPU/DCU 的情况下重放官方
`Eval/mia_th_COCO.py` 逻辑，得到 `alpha=0.55`、`ASR=0.883`、`AUC=0.941948`、
`TPR@1%FPR=0.488`、`TPR@0.1%FPR=0.324`。Direction A 主稿现在把 MoFit 作为
高分 public score-surface replay candidate 放入 negative/support 表，并新增
`versions/direction-a-mofit-public-score-surface.md` 到 anonymous supplement。

同轮继续做 dedicated artifact follow-up：`E2Q-020` LAION-MI 的 Hugging Face
dataset surface 当前仍只有 `members=13,396`、`nonmembers=26,874` 的
`url`/`caption` split rows，没有 public score rows、generated responses、
ROC/metric JSON、target checkpoint hash、surface-delta control 或 no-training
verifier；`E2Q-013` R125 DreamBooth / `ronketer/diffusion-membership-inference`
当前 HEAD 为 `eb2df6fdfaddeefbabdd1dc1f04b9dee32174ed4`，latest commit 只是
README 组织更新，公开 notebook 虽有六个 reconstruction-loss 标量，但目标
LoRA/checkpoint 和 forensics query set 仍绑定私有 GDrive 路径，没有 public
member/nonmember manifest、query-label-score join、ROC/metric JSON 或 verifier；
`E2Q-022` MIDM 当前 GitHub HEAD 为
`a7e7be0e00da5ea9473a0e9e1d0091fec638c8c0`，README SHA-256 为
`55c17489e93b8ce375e24e11222fdc8afc189a2c7d791f3c44be9e113341326b`，
公开面是 preprocessing/attack/metric code 和 output-free notebook，没有
committed row manifest、target checkpoint hash、score/result packet、
ROC/metric JSON、notebook outputs 或 verifier。三者都只保留为 support /
watch-plus，不进入 second-asset、N50、C14、admitted evidence 或 compute
release。
后续 hardening 又让脚本输出 derived audit files：metrics JSON、alpha sweep、
best-alpha ROC rows、public file manifest、implicit position manifest、1000-sample
bootstrap CI 和 1000-sample permutation null；seed `20260608` 下 bootstrap AUC
95% CI 为 `[0.929991, 0.955904]`，permutation-null AUC mean 为 `0.500805`。
这仍不改变 admission：official public surface 仍缺 explicit score row IDs，
target checkpoint identity 仍是本地路径/非 immutable public hash，且缺 official
metric/ROC/control packet 和 surface-delta packet。MoFit 不进入 C14、N50
denominator、admitted evidence、second independent asset 或 compute release。

同轮 `E2SCT-029` / `osquera/MIA_SD` 又完成 dedicated public-surface check。
这次检查把它从泛化的 support-only pressure 收窄为 C14-v2 candidate
public-result-surface row：公开树确实有 result-like `.pkl`、`.pgf` 和 `.npy`
文件，静态 pickle-opcode inspection 能看到 `y_true`、`y_pred`、ROC arrays
等键，PGF 也暴露 `Mean ROC AUC = 0.86 +/- 0.02`。但 admission 仍不变：
实验图片明确未发布，row identities 不公开，target checkpoint identity 不是
immutable public artifact，pickle 也不是安全的 row-bound verifier 格式。
因此 `E2SCT-029` 不进入当前 C14 `13` 行、不进入 N50 denominator、不作为
admitted evidence、second public response/score asset 或 compute release。结论记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct029_mia_sd_public_surface_check_2026_06_08.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2sct029_mia_sd_public_surface_check_2026_06_08.md)。

同轮第三个 targeted public-artifact discovery pass 又复核了 `Stry233/SAMA`
和 `kaslim/LSA-Probe`。SAMA 是当前公开的 diffusion-language-model MIA
codebase，且源码能在本地运行后生成 per-sample metadata，但公开仓库没有提交
target identity、immutable member/nonmember row manifest、score/metadata packet、
ROC/metric JSON 或 no-training verifier；LSA-Probe 仍是 implementation-pending
music/audio watch item。因此 targeted post-queue discovery 已检查 `15` 个候选且
仍找到 `0` 个新的 public row-bound score/response artifact；C14、N50 denominator、
admitted evidence、second-asset status 和 compute release 均不变。

同轮第四个 targeted public-artifact discovery pass 又把 current-date R125
DreamBooth recheck 和 cross-modal/tabular scout 合并记录。`E2Q-013`
`ronketer/diffusion-membership-inference` 仍是 private-target course-notebook：
public notebook 有六个 reconstruction-loss scalars，但 target LoRA/checkpoint
和 query set 仍在私有 GDrive runtime path；MIA-KDE 是最近的 tabular weak
challenger，公开 tree 有 `*_MIA_Distances.7z` 且 README 覆盖 TabDDPM，但
no-download surface 没有 row-score CSV、ROC/metric JSON、manifest 或 verifier；
ImageAuditor 和 2022 T2I MIA OpenReview line 又分别停在 paper-only /
consumer-boundary watch 和 historical paper-only reference；Tab-MIA、DOMIAS、
Synthetic-Data-Privacy 也只作为 support/code 或 wrong-target reference。
strong-filename GitHub search 未找到新的 compact score packet。因此 targeted
post-queue discovery 已检查 `20` 个候选，仍找到 `0` 个新的 public row-bound
score/response artifact；C14、N50 denominator、admitted evidence、second-asset
status 和 compute release 均不变。

同轮第五个 current arXiv/GitHub primary-source refresh 又检查了当前容易重复触发
检索的 5 个标题：`2605.27020` SD-MIA、`2605.17500` Silent Brush、
`2604.19891` Data-Free FL hardware MIA、`2604.01481` DISCO-TAB 和
`2507.17066` Risk In Context。SD-MIA 的官方仓库 `wanghl21/SD-MIA` 仍停在
`89384223da4ef95f9bddb3d1e222ccf339b914ac`，没有超过既有
`E2SCT-028` 检查的公开 artifact surface；Silent Brush 仍是已关闭的
style-leakage / semantic-shift watch；其余三者分别是 hardware FL 或
tabular/ICL synthetic-data privacy surface，不是当前 image/latent-diffusion
row-bound denominator。结论记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_targeted_public_artifact_discovery_2026_06_08_e.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_targeted_public_artifact_discovery_2026_06_08_e.md)。
Pass E 新增 row-bound score/response artifact 为 `0`；C14、N50 denominator、
admitted evidence、second-asset status 和 compute release 均不变。

同轮又对 `E2Q-020` LAION-MI / `Towards More Realistic Membership Inference
Attacks on Large Diffusion Models` 做 dedicated artifact follow-up：当前
Hugging Face dataset SHA 为 `194fe331b99016a85381d9d9d20a65b3edbd76c9`，
repo tree 仍只有 `README.md`、`members` parquet 和 `nonmembers` parquet；
Dataset Viewer/API 暴露 `members=13,396`、`nonmembers=26,874` 和 `url` /
`caption` 两列。它仍是强 L1 split artifact，但不是第二 public score/response
asset：没有 score rows、generated responses、ROC/metric JSON、target checkpoint
hash、surface-delta control 或 no-training verifier。结论记录在
[`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2q020_laion_mi_dataset_surface_followup_2026_06_08.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2q020_laion_mi_dataset_surface_followup_2026_06_08.md)。
因此 C14、N50 denominator、admitted evidence、second-asset status 和 compute
release 均不变。

当前允许的升级路径只有三类：

1. larger preregistered distinct-surface corpus + external label audit +
   false-promotion baseline study，用来证明 evidence contract 相比 score-only、
   code-only、artifact-availability 规则确实减少 false claim promotion；
2. second independent response/score asset，必须有 target/split identity、
   row-bound response or score packet、metric provenance、label-shuffle /
   permutation controls 和 surface-delta boundary；
3. report-correctness / systems evidence，必须有 report-renderer A/B drift
   reduction、external use、deployment evidence 或 public-safe evaluator telemetry。

DCU/scnet DDPM 线当前只作为 weak negative/support evidence：TC64/TC128/TC192 的
SecMI-style DDIM trajectory score 和 TC128/TC192 gradient-norm/loss/noise-norm
结果均在弱区间。最终 focused TC192 e18 后处理已完成：DDIM best AUC `0.5234`
且 95% CI `[0.4870, 0.5581]` 跨 `0.5`；gradnorm best AUC `0.5376`，95% CI
`[0.5022, 0.5725]`，幅度仍不足以支撑论文升级，且 untrained norm control 也到
`0.5303`。该线已关闭为 DCU-scale self-trained CIFAR-10 DDPM 的边界证据。
不得继续 TC64/TC128/TC192 epoch、seed、timestep、DDIM/LR/noise/loss matrix，
也不得把这条线包装成主贡献。

### 2026-06-06 七天执行板

本周目标不是“继续优化论文”或“补实验矩阵”，而是证明至少一个能改变论文等级
的证据对象是否存在。若下列两条都失败，Direction A 继续作为 bounded
measurement paper 推进，不再声称 CCF-A ready。

1. **E2 corpus freeze path**：用 no-download public-source refresh、
   duplicate review、six-gate annotation 推进 corpus freeze。当前只实审
   `E2Q-005` 的 feature-packet-only acceptability；使用
   [`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2q005_external_style_review_rubric_2026_06_06.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2q005_external_style_review_rubric_2026_06_06.md)
   和同目录 CSV template。3-reviewer 聚合已完成：
   [`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2q005_external_style_review_aggregation_2026_06_06.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2q005_external_style_review_aggregation_2026_06_06.md)。
   结论是 `accept_feature_packet_review_row / bounded-support`，且
   `target=Partial`、`provenance=Partial`；`E2Q-005` 不进入 N50 denominator。
2. **E1 CopyMark manifest-first gate**：当前 authenticated no-download recheck
   已完成，见
   [`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2q006_copymark_compact_manifest_gate_2026_06_06.md`](docs/internal/e2-n50-freeze-preflight-2026-06-06/e2q006_copymark_compact_manifest_gate_2026_06_06.md)。
   结论仍是 no-go：公开 GitHub/HF 面没有 compact manifest 或 no-training
   verifier；不下载 HF zip、图片、模型，不跑 PIA/PFAMI/SecMI/GSA。只在
   GitHub commit / HF dataset SHA / 作者 manifest 发生变化后重查。
3. **Direction A gate decision**：到本轮结束必须明确继续押
   larger corpus + external audit + false-promotion baseline，还是转向
   second independent response/score asset。若两者都没有 row-bound 证据，
   则停止 CCF-A claim，把主稿定位为 bounded measurement submission path。

### 2026-06-06 Compute Policy

- Day 1-2：只做证据审查和表结构校验；禁止 GPU/DCU、大下载和训练。
- Day 3-4：只做 CPU/package work：`E2Q-005` 单行审查、CopyMark compact
  manifest check、CLiD row-identity check、MT-MIA stratum decision、E2 freeze
  schema gate。当前这些 gate 已闭合，且没有一项释放 GPU/DCU 或大下载。
- Day 5：只允许一个小规模 bounded run，且必须先满足
  target/checkpoint identity、exact split/query IDs、row-bound score/response
  packet plan、metric provenance、label-shuffle/permutation control、
  consumer/delta boundary 和 duplicate/stratum policy。
- Day 6：只评估结果。若 AUC 接近随机、CI 跨 `0.5`、low-FPR 为 `0`，
  或 untrained/control 同量级，立即关闭，不追加 sweep。
- Day 7：长跑审批日。只有小跑达到 paper-changing threshold、controls 随机、
  row-bound packet 可准入，才允许长跑；否则继续 evidence-only。

新增 CPU guard：
[`scripts/check_e2_freeze_preflight.py`](scripts/check_e2_freeze_preflight.py)
已接入 `scripts/run_pr_checks.py`，用于防止把 scout/candidate rows 误写成
...
- `paper_portfolio.md`：四个候选论文方向和研究团队分工。Direction A
  ...
  是 H2-family response-cloud observable 短文候选；Direction C artifact
  claim-support 是可扩展测量论文候选，不写成 pooled reproducibility rate；
  Direction D consumer-boundary artifact/demo/report-correctness paper 是后续方向。
  2026-05-27 多团队决策进一步明确：允许生成多版论文，但只作为受控 writing
  tracks，不作为独立 evidence fork；A 是唯一当前 LaTeX 主稿，B/C/D 保持
  Markdown manuscript-level tracks；当前不开 Direction E，因为没有第五个能独立
  支撑科学 thesis 的证据对象。
...
- `multi_direction_paper_drafts.md` 和 `versions/drafts/`：已把四个方向扩成
  ...
  是 response-cloud observable short/workshop paper，不能声称跨模型/跨数据集
  portability；Direction C 是 selected-corpus claim-support paper，当前已有
  v1 corpus、fixed-search batch、broader-source API pass、targeted artifact-link
  pass 和 gate-summary 资产，
  若要独立投稿还需更大 distinct-surface frozen corpus，若声称
  reliability/adjudication 还需外部标签审查；
  Direction D 是 artifact/demo/report-correctness paper，已有 generated risk
  card 和 selected observed fault-injection rows；若要 systems paper 仍需
  report-drift、deployment、external-use 或 user-study 证据。
...
- `papers/diffaudit-evidence-paper/data/claim_trace.csv` 和
  `papers/diffaudit-evidence-paper/data/source_provenance.csv`：2026-05-26
  claim-trace hardening 后，主稿不再只依赖三份人工 ledger 视图，而是生成逐
  claim trace 记录和本地 artifact 身份表。每条主 claim 绑定 source artifact、
  replay tier、availability tier、六个显式 gate columns、first blocker、allowed wording；
  provenance 表记录本地 SHA-256、repo commit、tree state、generator command、
  source kind 和 availability tier。`build_paper_assets.py` 已把该 schema 固化为
  生成期校验：gate 列只能是 `Pass/Partial/Fail/N/A`，每个 `provenance_id`
  必须解析到带 hash 的 source-provenance 行。
...
- `scripts/export_admitted_evidence_bundle.py` 和
  ...
  target-score defense bridge，不再写成 `runtime-mainline`。
- `main.tex`、`refs.bib` 和 `paper.pdf`：IEEEtran 会议格式稿件已通过
  ...
  claim-first gate adjudication 流程、reportable row 的 main/comparator/bridge
  角色标注；2026-05-26 追加 reviewer-risk hardening 后，metric gate 明确拆成
  row-score replay、target-score replay、response/feature replay 和 source-documented
  point-estimate tiers，gate matrix 不再把五个 reportable rows 合成单一 mixed-pass
  行，reportable-row 图也改为按 access / claim role 固定排序并在标签中显式标注
  replay tier 的 evidence inventory，而不是 metric-ranked leaderboard；H2 在
  claim register / 图注 / 正文中统一写成 failed-admission stress test；
  `admitted-evidence-bundle.json` 也补了 `required_access` 和
  ...
  最新 claim-trace pass 又把五行报告口径显式拆成 replay-admitted rows 和
  source-documented point estimates，后者不能支持 interval/dominance
  claims；把过宽的 reproducibility-boundary 口径收紧为 paper-workspace
  replay boundary；并把 Direction C second-pass review 降级为 internal
  label-hygiene，而不是独立 reliability evidence。
  ...
  count summary、Reportable Bundle、H2 case study、H2 admission-decision table、
  Negative/Support Evidence、decision-value table、Threats to Validity 和
  Discussion/Conclusion。最新 publishability pass 又补入 security-science /
  benchmarking flaw 引用，把 `verifier checks` 改成 protocol checks，并压缩
  selection-boundary、H2 和 threats prose。最新 reviewer-stress pass 进一步把
  H2 gate matrix 对齐 claim trace 的 consumer/delta `Fail/Fail`，把五行表改成
  bounded paper roles，收紧 recon point-estimate 角色，并将 selected-corpus
  gate-count 图移出主文改为正文摘要，使 PDF 仍保持 `8` 页。
  2026-05-26 multi-team paper-version pass 之后，`versions/README.md` 被明确为
  A-D 论文方向和研究团队的唯一权威版本板，`paper_portfolio.md`、
  `multi_direction_paper_drafts.md` 和 `research_team_pitches.md` 降级为
  reader-facing views。四个方向的边界也已收紧：Direction A 主稿改成
  audit-validity measurement 语气，Direction B 默认写作对象改为 H2-family
  response-cloud observable 而不是 general mechanism / product attack，
  Direction C 的 fixed-search batch 改写为 separately frozen GitHub/arXiv
  metadata pass 并增加 denominator preamble，Direction D 降级为 Markdown
  artifact/demo preparation，未有 public-safe risk card 和 observed
  fault-injection table 前不得声称 submission-ready 或 systems effectiveness。
  2026-05-27 PDF visual/readability pass 进一步把主稿剩余工程口吻替换成
  downstream/audit-boundary 论文口径：abstract、gate matrix、reportable table 和
  H2 case study 不再使用 product/runtime admission、claim-promotion、
  runtime-smoke、mechanism-probe 等容易触发项目治理感的主文措辞。
  同日的 multi-team paper pass 又把 abstract/introduction/contribution bullets
  收紧为 claim-bound evidence packet、measurement object 和 admission rule 口径，
  并把 `versions/README.md` 固化为 A-D 四队受控 writing tracks：A 继续唯一
  LaTeX 主稿，B/C/D 保持 Markdown manuscript-level tracks，不开第五方向。
  随后的 agent-team reviewer-stress pass 把主稿进一步改成结果先行：abstract 和
  introduction 先报告 `12` 条 claim trace、`3` 个 replay-admitted packet、`2`
  个 source-documented point comparator，以及 `21` 行 selected artifact records
  + `17` 行 fixed-search trace 的 claim-control 用途；Measurement Protocol 明确
  `claim_trace.csv` 和 `source_provenance.csv` 是本稿冻结 sidecar，改 headline
  claim 必须改 trace row 和 allowed wording。纸面数据层也去掉旧
  `runtime-smoke` 口径：`admitted_rows.csv` 现在导出 `reportable-mainline` 和
  `target-score-defense-bridge`，`source_map.md`/`evidence_bank.md` 同步使用
  consumer-boundary / target-score defense bridge 口径。
  继续的 corpus-construction pass 针对审稿风险“selected corpus 不够透明”：
  主稿新增 compact construction table，明确 `artifact_corpus_v1.csv` 的 `21`
  行 evidence-note denominator、2026-05-26 fixed metadata batch 的 `7` 个
  GitHub repository queries + `5` 个 arXiv API metadata queries、`17` 行记录、
  `0` 个新 admitted rows，以及这些计数只支持 declared-set claim-control，
  不支持 field prevalence、artifact-quality 或 reproduction-rate 估计。
  后续 claim-support stratification pass 在 `build_paper_assets.py` 中生成
  `artifact_claim_support_rows.csv` 与 `artifact_claim_support_summary.csv`：
  `21` 行 selected corpus 被编码为 L0/L1/L2/L3 = `8/3/9/1`，`17` 行
  no-download fixed-search batch 被编码为 `13/4/0/0`。主稿 corpus 段已改为
  直接报告这些分层计数，明确 fixed-search 不含 L2/L3 replay/admission row，
  从而减少 metadata rows 被读成 replayability 或 auditability denominator 的风险。
  随后 H2 reviewer-risk pass 又新增 H2 admission-decision ledger，把
  output-cloud `AUC=0.9615`、raw/lowpass baseline、label-shuffle、shared-position/
  seed/cache-robustness controls 和 SD/CelebA img2img `25/25` portability failure 拆成
  metric signal、same-family robustness 和 admission boundary 三个问题；正文明确
  强同家族信号不能覆盖不足以准入的 portability packet 或缺失的
  consumer-boundary admission path，因此 H2 是 failed-admission stress test，
  不是新 admitted attack。
  2026-05-27 agent-team paper pass 后，Direction A 主稿把 C1-C13 trace
  分组成 reportable roles、H2、selected corpus、negative/support routes、
  uncertainty、source-breadth 和 targeted L1 artifact-link sidecars，并把详细 decision log 留在
  sidecar/版本板以守住 8 页；Direction B 统一降格为
  response-cloud observable/diagnostic，不再写 mechanism 或 transfer；Direction C
  draft 增加 denominator-first Results Skeleton；Direction D 增加 generated
  public-safe risk card、两级 demo/systems gate，并记录
  `scripts/export_admitted_evidence_bundle.py --check`、
  `scripts/render_admitted_risk_card.py --check` 和
  `scripts/check_public_surface.py` 于 2026-05-27 通过。继续的 D fault-injection
  pass 使用 `build/fault-injection/` 临时副本注入 candidate row、row removal、
  missing denominator 和 source drift，现有 export check 均以 `exit=2` 拦截；
  risk-card renderer 对 candidate replacement 和 missing nonmember denominator
  也以 `exit=2` 拦截，当前 admitted bundle 可成功生成五行 risk card；D draft
  另把 direct H2 / Tracing Roots candidate-promotion language guard 接入
  `scripts/check_public_surface.py` 和 `scripts/run_pr_checks.py`：当前扫描
  `main.tex` 与 generated risk-card markdown，并用 synthetic promotion phrases
  验证会触发 guard；最新 D packaging-hygiene pass 又把
  `workspaces/implementation/artifacts/admitted-risk-card.md` 从 `.gitignore`
  中放出来，`scripts/render_admitted_risk_card.py` 增加 private source-field
  拒绝逻辑，`run_pr_checks.py` 增加 tiny report-language A/B smoke（未受控 H2
  admission 句子会被 flag，当前 generated risk card 无 candidate-promotion 命中）。
  这补强 artifact/demo 包，不构成 renderer A/B drift reduction 或 systems
  effectiveness。D draft
  另新增静态架构图 `versions/drafts/direction-d-architecture.svg`，用于
  artifact/demo 包的 report-correctness flow 说明；
  并把既有 `docs/evidence/admitted-consumer-drift-audit-20260512.md` 与
  `docs/evidence/admitted-consumer-drift-audit-20260515.md` 纳入 D 证据链，
  作为 admitted-only consumer boundary 在两轮 candidate churn 中未漂移的
  report-boundary evidence。这仍不能写 renderer A/B drift reduction、
  external adoption、deployed enforcement 或广义 systems-effectiveness。随后主稿 Negative and
  Support Evidence 改成 route-local decision table，逐行列出 ReDiffuse、
  CommonCanvas、MIDST、Tracing Roots、collaborator SD ReDiffuse 和 fixed-search
  trace 的 packet/readout、first blocker 和 `Not allowed` 标签，避免弱结果被
  读成 cherry-picked prose 或反向过度结论。
  最新 title-framing pass 将 Direction A 标题统一为
  `A Claim-Admission Protocol for Diffusion Membership-Inference Scores: Boundary Cases and Evidence States`，
  明确论文对象是 claim/evidence 的 admission-state assignment，而不是泛化的 diffusion
  leakage auditing product 或项目治理工具。
  最新 multi-version team pass 继续按 A-D 四个研究团队推进：A 主稿把 abstract/
  introduction 改成 claim-state transition measurement，不再像 repository
  reproducibility checklist；Evidence Contract 中的 consumer-boundary gate 改为
  model owner、replaying auditor、downstream report 三类外部消费条件先行，
  allowed wording 是 gate 输出而不是 gate 定义；Case-Study Corpus 明确为
  purposive stress-test corpus，不估计 protocol sensitivity/specificity 或
  field behavior；H2 在主稿中只保留为 largest failed-admission example，
  standalone H2 口径归 Direction B。B/C/D 只读团队审查后同步到
  `versions/README.md`：B 只支持 H2-only short/workshop，C 只支持
  selected-corpus claim-support note 且 `21/17/10` denominators non-poolable，
  D 仍是 artifact/demo package，systems effectiveness 需要 report-renderer A/B
  drift、external-use 或 deployment evidence。
  2026-05-27 continuation reviewer-stress pass 继续收紧主稿 Introduction：将
  DiffAudit 的 endpoint 明确写成 falsifiable claim admission，审稿人可以检查
  trace row、first missing surface 和 allowed wording 是否随证据变化；同时把
  新增措辞压缩进原段落，避免把 PDF 推到 9 页。主稿已重编译回 `8` 页。
  后续 reference hygiene pass 核对 `main.tex` 的 `25` 个 citation：全部能在
  `refs.bib` 解析，unused keys 为 `0`，`bibtex .\build\main` 后 `main.blg`
  记录 `warning$ -- 0`；因此没有为了“补全”而强塞 URL/DOI，避免把 8 页稿挤爆。
  最新 Direction A readiness pass 已在
  `versions/drafts/direction-a-evidence-contract-paper.md` 填入 full-paper
  readiness memo：title、abstract/headline claims、contribution claims、visible
  figure/table objects、forbidden-claim scan、reference hygiene 和当前 go/no-go
  都集中到一个审稿面；结论是继续作为唯一 full LaTeX 主稿，但仍不声称
  CCF-B submission-ready。
  之后的 reviewer-stress wording pass 将 abstract、corpus table 和 conclusion
  中偏内部登记表的措辞改成 `case-study corpus`、`audited records` 和
  boundary-reporting 语言；重编译后仍为 `8` 页，PDF 文本层内部词扫描为 `0`。
  继续的 falsifiability pass 将 Discussion 中的纯否定式免责声明改为 proof-obligation
  口径：H2 需要 non-adjacent response asset 与 consumer-boundary admission path，
  selected-corpus aggregate 需要 larger distinct-surface frozen corpus 或 external
  label audit/adjudication，point-role comparator 需要 row/target-score arrays 才能写
  interval/dominance；该 pass 不提升任何 candidate row，只强化可证伪测量口径。
  后续 PDF font/layout QA pass 修正 `build_paper_assets.py` 的 matplotlib 嵌入方式，
  重新生成图表并重编译 `paper.pdf`；`pdffonts .\paper.pdf` 已不再出现 Type 3
  字体，输出为 LaTeX Type 1 字体加图表 CID TrueType 字体；`main.tex` 通过
  后续 validity-threat compression 后不再需要手动 `\IEEEtriggeratref` split，
  重编译页数仍为 `8`。
  同轮 final claim audit 将主稿 headline numbers 逐项对回 sidecar：`claim_trace.csv`
  是 C1-C13 共 `13` 条，`admitted_rows.csv` 是 `5` 行且 replay tier 分布为
  `2` 个 row-score replay、`1` 个 target-score replay、`2` 个 source-documented
  point estimates；H2 主结果、label-shuffle、same-family cache-robustness 和
  SD/CelebA img2img `0.7888` / zero-tail 边界均能在 `h2_output_cloud_rows.csv`
  与 Direction B evidence note 中找到；Direction C 的 selected-corpus 与
  fixed-search denominators 分别为 `21` 和 `17`。继续的 broader-source pass
  使用 no-download API 元数据方式补了 HF/Zenodo/OpenReview 三个外部源：
  `artifact_corpus_broader_source_20260527.csv` 记录 `10` 行 source-query
  observations，其中 HF 三个查询和 Zenodo 三个 title-exact 查询均为 `0`
  命中，OpenReview 在 stable-diffusion 查询中只返回一个已由 arXiv batch
  覆盖的 WACV/CoRR duplicate titled surface，新增 distinct artifact surfaces 为
  `0`，admitted rows 为 `0`。该 pass 只支持 source-coverage hygiene，不支持
  field prevalence、replayability 或 standalone corpus breadth。因此当前剩余主稿风险是
  venue-facing argument quality 和 selected-corpus framing，而不是缺一张
  分数表或 replay/source-tier 对账。
  继续的 targeted artifact-link pass 只在既有 metadata-only seeds 上追证据链接，
  记录 `2` 条 L1 split-artifact surfaces：HF LAION-MI split dataset 和 official
  SecMI GitHub implementation/split tree。它们没有 row-score、response/feature、
  ROC、metric JSON、checkpoint hash 或 verifier packet，因此只进入 C13 sidecar，
  不进入 replay/admission 或 pooled corpus denominator。
  随后的 selected-corpus framing pass 将主稿
  `Case-Study Corpus and Boundary Outcomes` 改成 declared-set measurement
  object 口径：先冻结 denominators 再读 counts，metadata-only fixed-search rows
  不与 replayed score packets 混池，`21`/`17` 只支持 claim-boundary control，
  不被写成 field prevalence、artifact-quality 或 reproducibility-rate。当前
  Direction A 的剩余论文风险进一步收窄为 reviewer-protocol polish，以及
  若要 standalone aggregate claim 时仍需更大的 frozen corpus 或 external
  adjudication evidence。
  本轮 multi-team tightening 进一步把四个论文方向分清：A 主稿把 C1-C13
  claim trace 分组外显、补 consumer-boundary rubric、补 corpus inclusion/exclusion
  rule、把 10-row HF/Zenodo/OpenReview source-breadth hygiene 和 two-row targeted
  L1 artifact-link sidecar 纳入 Direction C 版本板，并写明
  H2 repeated-stratified-holdout scorer 协议；B draft 新增 H2 positive/control
  与 SD/CelebA img2img failed-portability 两张主结果表；C draft 将旧
  reproducibility 文件名标为 legacy shorthand，新增 21/17/10 denominator-first
  corpus-flow；D draft 明确 selected fault-injection 只支撑 artifact/demo，
  systems-effectiveness 仍需 report-renderer A/B drift reduction、external use 或
  public-safe deployment/demo evidence。
  继续的 wording polish pass 清理主稿 paper-facing surface 中的内部项目词：
  `workspace`、`agent review`、`current evidence bank`、`DiffAudit route decisions`
  和 `Research evidence` 已从 PDF-facing 正文中移除或替换为 audited artifacts、
  bounded second-pass label review、audited evidence bank、audit route decisions
  和 candidate evidence；保留的命中只剩方法名或 guarded limitation wording。
  `BUILD.md` 记录了 `latexmk` 不可用时的手动编译链路。当前 `paper.pdf` 已按
  手动链路重编译为 `8` 页，PDF 字体扫描、PDF 文本公共扫描、LaTeX 关键告警扫描、
  PR checks 和 `git diff --check` 均通过。
  本轮 continuation QA 在最新 audit-claim admission framing 后再次执行完整手动
  LaTeX 链路，`pdfinfo` 确认 `8` 页，`pdffonts` 仍无 Type 3 字体，LaTeX log
  无 undefined citation/reference 或 overfull 命中，PDF 文本 public-surface
  扫描无私有路径、secret/token、product-claim wording、disprove 或 field-wide prevalence
  命中；`run_pr_checks.py`、`check_public_surface.py` 和
  `render_admitted_risk_card.py --check` 通过，`git diff --check` 只有 LF/CRLF
  提示。多论文方向继续固定为 A-D 四队：A 唯一 TeX 主稿，B 为 H2-only
  short/workshop，C 为 selected-corpus claim-support note，D 为 artifact/demo
  report-correctness package；不开第五方向，也不允许任何方向私建证据分叉。
  同轮只读研究组复核后，B 的独立性来自 response-cloud geometry + failed
  img2img portability gate，不需要同缓存 sweep；C 当前只够技术报告或 scoped
  measurement note，`21` 行由 `11` 个 v0 evidence surfaces 和 `10` 个 v1
  metadata-only expansions 组成，`17` 与 `10` 分别是 fixed-search provenance
  和 source-breadth hygiene，绝不能合成 `48` 或写成 pooled rate；D 仍是
  admitted-only risk card、selected fault-injection blocking 和 report-language
  guard 的 artifact/demo note，systems-paper 升格需要 report-renderer A/B
  drift、external/semi-external use、public-safe demo/deployment telemetry 或更完整
  report-surface fault-injection 证据。
  本轮 reviewer-surface pass 继续处理 A 稿第一眼审稿风险：标题改为问题导向，
  first page 去工具化；contribution bullets 改成 gate vector、first blocker、
  evidence state 和 strongest allowed wording 这类可证伪输出；Measurement Protocol
  先给 claim/packet 四步协议再写 sidecar 实现；selected corpus 改为 boundary-case
  corpus；H2 改成 high-AUC non-admission counterexample，避免像隐藏第二主贡献。
  重编译后 `paper.pdf` 仍为 `8` 页。
  最新 visual/team hardening pass 修掉 Figure 1 的项目内口吻：图题从旧的
  项目冠名前缀改为 `Evidence-contract pipeline`，
  分支文字从 Research/report-entry 语义改为 `not reportable here` 与
  `with bounded wording`。同轮 B/C/D 只读小队结论已落到现有 brief/draft：
  B 前置 candidate-only、finite empirical tail 和非 product/cross-model 口径；
  C 增加 Denominator Ledger，明确 `21`、`17`、`10` 不合成 `N=48`，且 L3 是
  role-level positive-control surface；D 明确 report-language A/B 只是 direct
  phrase smoke，不是 semantic verifier 或 measured drift reduction，并把 D
  brief/draft 纳入 `check_public_surface.py` 的 report-language 扫描。未开
  Direction E，也未新建 TeX fork。
  最新 Direction A external-reviewer pass 继续降内部流程感：摘要把 `17` 行写成
  metadata-only fixed-search trace，把 `10` 行写成 source-query hygiene；首个
  contribution 改为 falsifiable claim-admission measurement task；consumer-boundary
  gate 改为先固定外部 consumer action 和 required artifacts 再产生 bounded
  wording；H2 表题从 ledger 改为 admission-decision summary；reportable bundle
  图表补明 point rows 不携带 replay-derived intervals；H2 实现细节压缩为
  high-AUC non-admission stress test，避免像隐藏 attack paper。重编译后
  `paper.pdf` 仍为 `8` 页。

当前论文主线使用三类证据：

- reportable evidence bundle：`recon`、`PIA baseline`、`PIA defended`、`GSA`、
  `DPDM W-1` 五行，统一报告 AUC / ASR / TPR@1%FPR / TPR@0.1%FPR / cost /
  boundary；其中 PIA/PIA defended/DPDM W-1 是 replay-admitted，recon/GSA 是
  source-documented point estimates。
...
1. 继续把 Direction A 的 `8` 页稿件打磨成可投稿论文：压实 motivation、
   ...
   和 related-work 对照。
2. Direction C 已有 `21` 行 v1 corpus、`17` 行 2026-05-26 fixed-search
   metadata batch 和 `10` 行 2026-05-27 broader-source API observations，
   另有 `2` 行 2026-05-27 targeted artifact-link L1 seeds，分别来自 HF split
   artifact 和 SecMI official GitHub split tree；这些 targeted rows 不能并入
   `21/17/10` denominator，也不能写成 replay/admission 证据。
   并已完成 selected-corpus gate-label consistency pass 和
   ...
   measurement，不能写成“这些论文失败了”；若要独立成稿，再补更大
   distinct-surface fixed corpus 或外部独立标签审查。
...
当前 slots 不释放弱路线重型实验：
`active_gpu_question = manifest-gated CCF-A evidence object only`，
`next_gpu_candidate = none until a second asset or measurement-baseline protocol passes preflight`，
`CPU sidecar = paper asset generation / targeted artifact-link hygiene / CCF-A protocol drafting only`。
...
和 same-family cache-robustness review；
但它仍不晋升、
...
进一步的 CPU-only existing-cache robustness review 在两份 shared-position cache
...
`CPU sidecar = none selected after SD/CelebA img2img output-cloud portability review`。
See
...
SimA-style score-norm 单次评分都已完成。前者产出了 checkpoint 和 `512` 行
score packet，但固定 timestep denoising-loss 成员信号随机级；后者复用同一
checkpoint 和同一 `256 / 256` score split，`score = -L4_norm(model_epsilon_prediction_at_t100)`，
结果仍随机级。该线路不能释放下一轮 GPU 扩参，也不能作为第二资产或
Platform/Runtime 准入结果。

运行边界：官方 ReDiffuse DDPM `UNet` + `GaussianDiffusionTrainer`，STL-10
unlabeled，训练官方 member split 的 `1024` 个样本，`300` steps，batch `32`，
score `256` trained members + `256` official nonmembers，timesteps
`50/200/500/800`，`score = -mean_fixed_timestep_denoising_mse`。运行在
`diffaudit-research` CUDA 环境中，`92.750s` 完成，峰值显存 `2.430 GB`，
停止原因为 `step_budget`。

denoising-loss 指标：`AUC = 0.4996337890625`，`ASR = 0.509765625`，
`TPR@1%FPR = 0.01171875`，`TPR@0.1%FPR = 0.0`，非成员分母 `256`，
最小非零 FPR `0.00390625`。score-norm 指标：`AUC = 0.5052947998046875`，
`ASR = 0.525390625`，`TPR@1%FPR = 0.03125`，`TPR@0.1%FPR = 0.01953125`，
`15.938s` 完成，峰值显存 `0.365 GB`。这说明管线可执行、score packet
可生成，但该短程 target 的固定 timestep denoising-loss 和 denoiser-output
score-norm 都不携带可用成员信号。

因此当前 slots 收回为：
`active_gpu_question = none`，`next_gpu_candidate = none`，
`CPU sidecar = none selected after ReDiffuse STL-10 denoising-loss and score-norm weak results`。

不要把这两个弱结果扩成 step-count、seed、timestep、batch-size、subset-size、EMA、
scheduler、denoising-loss、norm、checkpoint-step 或 fusion 矩阵。只有出现公开第三方
STL-10 checkpoint/score packet、真正不同的成员 observable，或明确批准长训预算并
提前定义 checkpoint/score 发布合约时，才重新开启 ReDiffuse STL-10。See
...
- 官方 `STL10_train_ratio0.5.npz` 的 SHA-256 为
  `14a06133f36c74e7d3cb97dbe74385fb42c22335a7cb955fd9944ca503baca52`。
- STL-10 member/nonmember 各 `50000`，互不重叠，union 覆盖 `0..99999`，
  可绑定到本地 STL-10 unlabeled `100000` 行。
- `STL10_train_ratio0.5.npz` 与 `TINY-IN_train_ratio0.5.npz` byte-identical；
  这是 split provenance caveat，但不直接否定 STL-10 本身。
- `208` 个低层图像统计特征没有明显 label 泄漏：最强单变量 `auc_abs`
  约 `0.502`，线性 probe 在 `80000` holdout 上 `test AUC = 0.4994776215625`。
- CUDA 环境应使用 `conda run -n diffaudit-research python`，默认 PATH Python
  是 CPU-only。
- 官方 DDPM `UNet` + `GaussianDiffusionTrainer` 微训练校准通过：
...
`admitted-evidence-bundle.json` 的第六行。原因不是信号弱，而是消费语义不同：
该包是灰盒 feature tensor 级别证据，不是 raw checkpoint / image
query-response / per-image identity 证据。现有 bundle、导出脚本、测试和产品桥接
仍明确只支持五个准入行：`recon`、`PIA baseline`、`PIA defended`、`GSA`
和 `DPDM W-1`。

Tracing the Roots 保留为正面的 Research 候选和 feature-packet 消费方设计输入：
...
不从本次边界修正释放 GPU 训练或新下载。当前 slots：
`active_gpu_question = none`，`next_gpu_candidate = none`，
`CPU sidecar = none selected after feature-packet boundary correction`。
...
随后同一 checkpoint 上的 SimA-style denoiser-output score-norm 单次评分也随机级，
...
历史上最可达路径：
1. **ReDiffuse DDPM/STL-10**：同家族不同数据集。划分清单已存在，但 bounded scout 和 score-norm 评分已关闭该短目标 denoising-loss / denoiser-output norm 线路。
...
**2026-05-25 后续修正**：ReDiffuse 补充材料、STL-10 划分、CPU 侧预检、唯一
bounded scout 和同 checkpoint 的 SimA-style score-norm 单次评分已完成。该线路没有形成
...
Platform/Runtime 五行 consumer/reportable bundle。只有出现第二个公开非同源 feature-packet、
...
`Identity-Focused Inference and Extraction Attacks on Diffusion Models` as a
bounded metadata gate, not as a reproduction branch. The paper reports
membership inference, identity inference, and data extraction against diffusion
models in LFW / CelebA-style face settings, including MIA `AUC-ROC = 0.91`,
identity inference `92%` on LDM models trained on LFW, and DDPM data extraction
accuracy `91.6%`. This is useful identity-level privacy context, but it is not
the current per-sample image/latent-image membership audit contract.

The public surface is paper-source-only. The arXiv metadata exposes one visible
version, authors, abstract, PDF link, and source availability. The arXiv source
`HEAD` endpoint returns `application/gzip`, `Content-Length = 3,990,545`, and
SHA-256 ETag
`eedf38231ea31b6440f63109d7ab9fa3d49fa5bf3601703ac259d3e0405253e2`; the source
tarball was not downloaded. Exact-title and arXiv-id GitHub repository and code
searches found no official implementation repository, LDM / DDPM target
checkpoint bundle, LFW / CelebA identity or member/nonmember manifest,
generated response packet, per-row membership / identity score file, ROC array,
metric JSON, extraction-quality artifact, or verifier.
...
row`. Current slots become `active_gpu_question = none`, `next_gpu_candidate =
none`, and `CPU sidecar = none selected after Identity-Focused Inference /
Extraction artifact gate`. This does not change Platform/Runtime rows, schemas,
product copy, downloads, CPU sidecars, or GPU release. See
[docs/evidence/identity-focused-inference-extraction-artifact-gate-20260523.md](docs/evidence/identity-focused-inference-extraction-artifact-gate-20260523.md).

## 2026-05-23 RAPTA / ADMCD Copying-Mitigation Artifact Gate

Lane A/B checked arXiv `2603.13070` /
`Mitigating Memorization in Text-to-Image Diffusion via Region-Aware Prompt
Augmentation and Multimodal Copy Detection` as a bounded metadata gate, not as a
reproduction branch. The paper proposes Region-Aware Prompt Augmentation
(`RAPTA`) for prompt augmentation during training and Attention-Driven Multimodal
Copy Detection (`ADMCD`) for copying detection using local patch, global
semantic, and texture cues. This is relevant copying / memorization mitigation
context, but it is not the current per-sample image/latent-image membership
audit contract.

The public surface is paper-source-only. The arXiv HTML page exposes the title,
authors, abstract, and full-text links. The arXiv source `HEAD` endpoint returns
`application/gzip`, `Content-Length = 4,172,177`, and SHA-256 ETag
`f44be31acedbcb98485527bb56c4c1e7e02c96d646b7f09e36edf356280fe059`; the source
tarball was not downloaded. Exact-title, arXiv-id, `RAPTA ADMCD`, and
`Region-Aware Prompt Augmentation` GitHub repository and code searches found no
official implementation repository, target checkpoint bundle, copied/non-copied
or member/nonmember row manifest, generated response packet, per-row ADMCD score
file, ROC array, metric JSON, retained-utility artifact, or verifier.

Decision: `paper-source-only copying / memorization mitigation watch / no
official code / no score artifacts / no download / no GPU release / no admitted
row`. Current slots become `active_gpu_question = none`, `next_gpu_candidate =
none`, and `CPU sidecar = none selected after RAPTA / ADMCD copying-mitigation
artifact gate`. This does not change Platform/Runtime rows, schemas, product
copy, downloads, CPU sidecars, or GPU release. See
[docs/evidence/rapta-admcd-copying-mitigation-artifact-gate-20260523.md](docs/evidence/rapta-admcd-copying-mitigation-artifact-gate-20260523.md).

## 2026-05-23 GUARD Surgical Mitigation Artifact Gate

Lane A/B checked arXiv `2603.00133` /
`You Don't Need All That Attention: Surgical Memorization Mitigation in
Text-to-Image Diffusion Models` as a bounded metadata gate, not as a
reproduction branch. The official `kairanzhao/GUARD` repository is public,
MIT-licensed, and small enough to inspect through metadata at commit
`3a49dafe6de652c1a6d9b6dd13758d2e67118094`. GUARD is distinct inference-time
memorization-mitigation context: it combines attractive-repulsive generation
dynamics with cross-attention attenuation to guide generation away from known
memorized training images while preserving prompt alignment. It is not the
current per-sample image/latent-image membership audit contract.

The public surface is code-public but artifact-incomplete. The README identifies
the repo as official ICML 2026 code, uses the `sdv1_500_mem` Stable Diffusion
memorization benchmark, and points to Google Drive for prompts and ground-truth
images. The recursive GitHub tree exposes `detect_mem.py`, `generate_mask.py`,
`inference_mem.py`, metrics helpers, and vendored `open_clip` code, but no
checkpoint-bound target identity, immutable member/nonmember or
memorized/non-memorized row manifest, generated response packet, pre/post GUARD
score rows, ROC arrays, metric JSON, retained-utility artifact, or no-training
verifier. The arXiv source `HEAD` endpoint reports `application/gzip`,
`Content-Length = 1,429,877`, and SHA-256 ETag
`7d33099de25263768026fd4d5d45cbb825acbe01bac60ee648843bae565ba625`; the source
tarball was not downloaded.

Decision: `official code-public mitigation watch / no score artifacts / no
download / no GPU release / no admitted row`. Current slots become
`active_gpu_question = none`, `next_gpu_candidate = none`, and `CPU sidecar =
none selected after GUARD surgical mitigation artifact gate`. This does not
change Platform/Runtime rows, schemas, product copy, downloads, CPU sidecars, or
GPU release. See
[docs/evidence/guard-surgical-mitigation-artifact-gate-20260523.md](docs/evidence/guard-surgical-mitigation-artifact-gate-20260523.md).

## 2026-05-23 BAF LoRA Parameter-Space Mitigation Gate

Lane A/B checked arXiv `2605.10439` /
`Filtering Memorization from Parameter-Space in Diffusion Models` as a bounded
metadata gate, not as a reproduction branch. The paper proposes
Base-Anchored Filtering (`BAF`), a post-hoc, training-free, data-free
memorization mitigation method for diffusion LoRAs. It decomposes LoRA updates
into spectral channels, retains channels aligned with the pretrained backbone
principal subspace, and suppresses weakly aligned channels as possible
memorization carriers. This is a real weight-only LoRA mitigation surface, but
it is not the current per-sample image/latent-image membership contract.

The public surface is supplementary-code-claim-only. The arXiv API and HTML
page expose the title, authors, abstract, and the statement that code is
available in supplementary material; no public GitHub URL is visible. The
arXiv source `HEAD` endpoint returns `application/gzip`, `Content-Length =
5,785,836`, and SHA-256 ETag
`1d10717f5eb4f9ea99d8f36ce0d044e68a937aa88376af20c9d2000a04f6904a`, but the
source tarball and supplement were not downloaded. Exact-title, arXiv-id,
`Base-Anchored Filtering`, and BAF/LoRA/memorization GitHub repository and code
searches found no official repository, target LoRA/checkpoint bundle, training
image manifest, member/nonmember rows, generated response packet, per-row
memorization score, ROC array, metric JSON, retained-utility artifact, or
verifier.

Decision: `weight-only LoRA mitigation watch / supplementary-code claim only /
no public score artifacts / no download / no GPU release / no admitted row`.
Current slots become `active_gpu_question = none`, `next_gpu_candidate =
none`, and `CPU sidecar = none selected after BAF LoRA parameter-space
mitigation gate`. This does not change Platform/Runtime rows, schemas, product
copy, downloads, CPU sidecars, or GPU release. See
[docs/evidence/baf-lora-parameter-space-mitigation-gate-20260523.md](docs/evidence/baf-lora-parameter-space-mitigation-gate-20260523.md).

## 2026-05-23 Broken Memories Artifact Gate

Lane A checked arXiv `2605.22050` /
`Broken Memories: Detecting and Mitigating Memorization in Diffusion Models
with Degraded Generations` as a bounded public metadata gate, not as a
reproduction branch. This is fresh Stable Diffusion memorization
detection/mitigation evidence: the paper reports SD `1.4` memorization
detection with `AUC > 0.999`, post-mitigation memorization rate `0.0%`, and
about `0.01` seconds overhead per image. The setup uses `500` memorized
prompts from known duplicated LAION images constructed by Webster, nonmember
and reference prompt pools from Lexica / COCO / GPT-4 / LAION-400M, and a
fine-tuned SD `1.4` LAION subset setup from prior memorization work.

The public surface is paper-source-only. The arXiv API exposes title, authors,
`cs.CV`, and `KDD 2026, extended version`; the arXiv source `HEAD` endpoint
returns `application/gzip`, `Content-Length = 24,383,310`, and SHA-256 ETag
`e860fea66b3b44ecaa80f001c8443740443711151958005fe11ae82ec1d70c9d`, but the
source tarball was not downloaded. arXiv metadata and exact-title / arXiv-id
GitHub repo and code searches expose no official Broken Memories repository,
prompt manifest, generated image packet, internal update-norm trace, per-row
score file, ROC array, metric JSON, mitigation-decision artifact, or verifier.

Decision: `memorization detection/mitigation semantic-shift watch /
paper-source-only / no official code / no score artifacts / no download / no
GPU release / no admitted row`. Current slots become
`active_gpu_question = none`, `next_gpu_candidate = none`, and
`CPU sidecar = none selected after Broken Memories artifact gate`. This does
not change Platform/Runtime rows, schemas, product copy, downloads, CPU
sidecars, or GPU release. See
[docs/evidence/broken-memories-artifact-gate-20260523.md](docs/evidence/broken-memories-artifact-gate-20260523.md).

## 2026-05-23 IAR Privacy Attacks Artifact Gate

Lane A checked arXiv `2502.02514` /
`Privacy Attacks on Image AutoRegressive Models` as a bounded public metadata
gate, not as a reproduction branch. This is strong image-generation privacy
evidence with explicit membership inference, dataset inference, and extraction
claims, but the target family is image autoregressive generation rather than
the current DiffAudit diffusion / latent-image consumer contract.

The official `sprintml/privacy_attacks_against_iars` repository is code-public
and small enough to inspect through metadata: it exposes `main.py`,
`environment.yaml`, MIA/DI/memorization analysis scripts, VAR/RAR/MAR model
configs, and attack implementations under `src/attacks/`. The paper reports
IAR `TPR@FPR=1% = 94.57%` versus `6.38%` for comparable diffusion-model
attacks, dataset inference with as few as `4` samples, and `698` extracted
training samples from `VAR-d30`. The public tree still does not commit model
hashes, immutable ImageNet train/validation row manifests, generated sample
packets, per-row MIA scores, ROC arrays, metric JSON, DI CSVs, memorization
CSVs, or a no-training verifier. The README execution path requires cloning
upstream VAR/MAR/RAR-related repositories, downloading ImageNet train/val, and
letting scripts download models before generating outputs locally.

Decision: `image-autoregressive privacy watch-plus / official code-public / no
committed score packet / large ImageNet and model assets required / no download
/ no GPU release / no admitted row`. Current slots become
`active_gpu_question = none`, `next_gpu_candidate = none`, and
`CPU sidecar = none selected after IAR privacy artifact gate`. This does not
change Platform/Runtime rows, schemas, product copy, downloads, CPU sidecars,
or GPU release. See
[docs/evidence/iar-privacy-attacks-artifact-gate-20260523.md](docs/evidence/iar-privacy-attacks-artifact-gate-20260523.md).
...
for Trajectory Generation` as a bounded public metadata gate, not as a
reproduction branch. The paper is cross-domain trajectory/mobility privacy
evidence: it evaluates membership inference against LSTM-TrajGAN, MoveSim,
DiffTraj, and Diff-RNTraj. The two diffusion trajectory models use a
loss-based denoising-MSE membership score over `50` random timesteps, but the
reported diffusion results are near random: DiffTraj `AUC-ROC = 0.5012` and
Diff-RNTraj `AUC-ROC = 0.4949`. The only clearly positive table value is the
GAN MoveSim result, `AUC-ROC = 0.7002`.

The public surface is paper-source-only. arXiv `2605.15246v1` has a `106,622`
byte source tarball with SHA-256
`EA94CCE4DB20634E4B1E776AA09D8E2E3F2D5B6D402057DD33CB4BFE6AAABEF4` and `5`
entries: `00README.json`, `IEEEtran.bst`, `IEEEtran.cls`, `main.tex`, and
`ref.bib`. Exact-title, arXiv-id, author/title, and trajectory-generation
GitHub repository/code searches returned no official repository, code, score,
or artifact hits. No model checkpoint, immutable trajectory member/nonmember
manifest, generated trajectory packet, per-row score file, ROC array, metric
JSON, or verifier is public.

Decision: `cross-domain trajectory paper-source-only watch / diffusion
trajectory MIA weak / no code-score artifact / no download / no GPU release /
no admitted row`. Current slots become `active_gpu_question = none`,
`next_gpu_candidate = none`, and
`CPU sidecar = none selected after trajectory generation privacy artifact
gate`. This does not change Platform/Runtime rows, schemas, product copy,
downloads, CPU sidecars, or GPU release. See
[docs/evidence/trajectory-generation-privacy-artifact-gate-20260523.md](docs/evidence/trajectory-generation-privacy-artifact-gate-20260523.md).

## 2026-05-23 Model Will Tell DRC Artifact Gate

Lane A/B checked arXiv `2403.08487` /
`Model Will Tell: Training Membership Inference for Diffusion Models` as a
bounded public metadata gate, not as a reproduction branch. The paper proposes
Degrade Restore Compare (`DRC`): intentionally degrade an image, restore it
with the target diffusion model, and compare the restored image with the
original via semantic similarity. This is mechanism-relevant because it tests a
restoration-prior observable rather than raw denoising loss, final-layer
gradient, frequency cutoff, or same-noise residual variants.

The public surface is still paper-source-only. arXiv `2403.08487v1` has a
`1,745,140` byte source tarball with `19` entries: TeX, bibliography/style
files, and figure PDFs only. The paper says the authors intend to release code,
but no official code URL is present. Exact-title, DRC phrase, author-name, and
arXiv-id GitHub repository/code searches returned no official repository or
code hits. The paper reports strong DRC table metrics, including CelebA
`AUC = 0.989`, `TPR@1%FPR = 80.46%`, and `TPR@0.1%FPR = 54.52%`, but those are
paper-source metrics only. The described Cifar10/Cifar100/CelebA random
half-train splits and FFHQ whole-dataset member approximation are not released
as immutable row manifests.

Decision: `paper-source-only DRC restoration MIA watch / no official code / no
...
This does not change Platform/Runtime rows, schemas, product copy, downloads,
CPU sidecars, or GPU release. See
[docs/evidence/model-will-tell-drc-artifact-gate-20260523.md](docs/evidence/model-will-tell-drc-artifact-gate-20260523.md).

## 2026-05-23 Discrete DLM Withdrawn Artifact Gate

Lane B/watch checked arXiv `2605.16445` /
`Membership Inference Attacks on Discrete Diffusion Language Models` as a
bounded public metadata gate, not as a reproduction branch. The current arXiv
record is withdrawn, has no current PDF, and says citations and co-authors need
verification before a future version. The abstract reports Masked Diffusion
Language Model membership metrics, including MIMIR-domain mean `AUC = 0.878`,
...
not change Platform/Runtime rows, schemas, product copy, downloads, CPU
sidecars, or GPU release. See
[docs/evidence/discrete-dlm-withdrawn-artifact-gate-20260523.md](docs/evidence/discrete-dlm-withdrawn-artifact-gate-20260523.md).
...
Lane A returned to the official public CopyMark support surface because the
collaborator Stable Diffusion ReDiffuse bundle was useful candidate evidence
but not a public immutable asset. The selected bounded question was whether the
public `laion_mi` branch could clear the `public identity / exact split / row
binding` gate without the full CopyMark Hugging Face zip or a GPU rerun. The
local bounded package used the public `antoniaaa/laion_mi` member metadata,
official CopyMark `pia_laion_mi_image_log.json`, and the official member
download utility.

Decision: `blocked / public row-binding gap / no download beyond small metadata
subset / no GPU release / no admitted row`. The new
`python -m diffaudit probe-copymark-laion-mi-assets` command reproduces the
gate locally. On the current public surface, `members.parquet` exposes only
`['url', 'caption']` with `13396` rows, while the official member utility still
expects a hidden third parquet column via `df.iloc[idx, 2]`; direct access on
the current public parquet raises `IndexError`. The official member image log
contains `2500` numeric filenames with min id `9617` and max id `33905220`, so
they cannot be simple row indices into the current public parquet. A live
spot-check of the first ten public member URLs returned only `4 / 10` HTTP
`200` responses, so even URL rehydration is partial. Keep CopyMark as
Research-side support evidence only; do not download the full HF zip, image
payloads, model folders, or launch GPU work from this branch. Current slots
remain `active_gpu_question = none`, `next_gpu_candidate = none`, and
`CPU sidecar = none selected after CopyMark laion_mi public binding gate`. See
...
value and is treated as threshold-grid rounding. The mainline also now has
`python -m diffaudit probe-rediffuse-sd-assets` for the collaborator
source/detector bundle and
`python -m diffaudit score-rediffuse-sd-image --image <path> --prompt <text>`
as a candidate-only wrapper around the collaborator single-image scorer. The
current default local interpreter still lacks `fire`, `pytorch_lightning`,
`skimage`, and `omegaconf`, and the 2026-05-17 probe did not find a local
`CompVis/stable-diffusion-v1-4` cache, so direct scoring is wired but not yet a
selected runtime task. This bundle is useful mainline candidate evidence
because it is a real `5000`-row score packet and now has a first-class CLI
import path, but it does not satisfy the current public Lane A reopen gate: it
is a collaborator local transfer, the member side is a LAION-like repeatable
subset rather than the exact paper LAION-5B split, and the boundary is
local-model-query black-box rather than strict external API-only black-box. Do
not request `coco_data`, do not download Stable
...
Lane A 继续检查非重复候选 `osquera/MIA_SD`。该历史 verdict 已被
2026-06-08 `E2SCT-029` dedicated public-surface check 扩展：当前状态是
C14-v2 candidate public-result-surface only，仍不 admission、不 release compute。
该 repo 是 face-image
...
2026-06-08 follow-up: the full ZIP was downloaded only as a bounded artifact
probe under `Download/shared/supplementary/`, verified against Zenodo MD5, and
passed ZIP integrity. Static inspection of the nested torch dataset payloads
found only `image`/`text` fields and no immutable row IDs; the source code
generates score files separately, and those files are not present in the ZIP.

Decision: `archive-verified / manifest-incomplete / no GPU release`. This
candidate stays on Lane A watch, but LoRA scorer or GPU work is blocked until a
public README, appendix, code reference, or tiny manifest proves target
identity, exact split semantics, row IDs, and a score/metric packet.
See
...
`10.5281/zenodo.13371475`. The NDSS paper and
`py85252876/Reconstruction-based-Attack` repository confirm a real
...
The 2026-06-08 bounded full-ZIP probe verified the archive and source clone, but
the missing gate did not change: no public readable target member/nonmember
split manifest, immutable row IDs, ready scoring contract, or shipped score
packet was found, and the repository does not bind the Zenodo `dataset.pkl`
payloads to exact target sample identities.

Decision: `code-reference-found / full-probe-still-split-and-score-blocked /
no GPU release`. Zenodo remains watch-only; do not run LoRA scoring unless a
public manifest or repository file exposes exact split semantics, row IDs, and a
row-level score/metric packet. The next autonomous cycle should switch away from
Zenodo if no new external evidence appears. See
...
### 2026-05-13 MoFit artifact verdict（已被 2026-06-08 public score-surface preflight 更新）
...
marginal scoring 和 midfreq same-contract repeat。该段是 2026-05-13 的历史
verdict：当时公开仓库仍未暴露可复核代码/结果面。2026-06-08 后该判断已被
`docs/internal/e2-n50-freeze-preflight-2026-06-06/e2_mofit_public_score_surface_preflight_2026_06_08.md`
更新：官方 repo 现在有 compact COCO public score-like text replay，且主稿已把
它作为 high-AUC support/candidate row 纳入 negative/support 表；但由于缺 explicit
score row IDs、immutable public target identity、metric JSON/ROC packet 和
permutation/surface-delta controls，仍不得写成 Platform/Runtime admitted row、
N50 denominator、second public asset 或 compute release。历史 verdict 见
...
4. **历史 GPU release 规则已关闭**:2026-05-13 的 CommonCanvas P0 曾要求在
   manifest 过关后直接释放一次 bounded GPU run；该路线已被后续弱结果和当前
   `active_gpu_question = none` 覆盖，不是当前 compute-release 指令。
...
| Zenodo fine-tuned diffusion asset | watch / split-manifest blocked | Public paper/code references and a verified full-ZIP probe confirm a reconstruction-based attack workflow, but exact target member/nonmember sample identities, complete split semantics, and score packets are still not exposed; no same-line audit, LoRA scoring, or GPU release. |
...
| MoFit artifact verdict | historical 2026-05 code-TBW verdict superseded by 2026-06-08 public COCO score-surface preflight; now a high-AUC support/candidate replay, still not admitted/N50/second-asset evidence or compute release | [docs/evidence/mofit-artifact-verdict-20260513.md](docs/evidence/mofit-artifact-verdict-20260513.md) |
...
| Zenodo fine-tuned diffusion code reference audit | paper-and-code-backed watch plus verified full-ZIP probe, but split manifest, row IDs, and score packet are still missing; no GPU release | [docs/evidence/zenodo-code-reference-audit-20260513.md](docs/evidence/zenodo-code-reference-audit-20260513.md) |
| Zenodo fine-tuned diffusion asset verdict | archive-verified but manifest-incomplete; no scoring, admission, or GPU release | [docs/evidence/zenodo-finetuned-diffusion-asset-verdict-20260513.md](docs/evidence/zenodo-finetuned-diffusion-asset-verdict-20260513.md) |
...
to use only the five reportable bundle rows (`recon`, `PIA baseline`,
`PIA defended`, `GSA`, and `DPDM W-1`) with their stated replay/source tiers and keep ReDiffuse, SecMI stat/NNS,
