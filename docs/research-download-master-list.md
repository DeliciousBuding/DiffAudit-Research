...
> Current local/manual status snapshot: `docs/research-download-current-status.md`
> Teammate handoff guide: `docs/data-and-assets-handoff.md`
...
- `<DIFFAUDIT_ROOT>/Download/gray-box/supplementary`
- `<DIFFAUDIT_ROOT>/Download/white-box/datasets`
- `<DIFFAUDIT_ROOT>/Download/white-box/weights`
- `<DIFFAUDIT_ROOT>/Download/white-box/papers`
- `<DIFFAUDIT_ROOT>/Download/white-box/supplementary`
- `<DIFFAUDIT_ROOT>/Download/shared/datasets`
- `<DIFFAUDIT_ROOT>/Download/shared/weights`
- `<DIFFAUDIT_ROOT>/Download/shared/papers`
...
|---|---|---|---|---|---|---|
| `SH-DS-01` | CIFAR-10 python archive | Common source dataset for gray-box and white-box pipelines | [cs.toronto.edu CIFAR-10](https://www.cs.toronto.edu/~kriz/cifar-10-python.tar.gz) | No | ~170 MB | `<DIFFAUDIT_ROOT>/Download/shared/datasets/cifar-10-python.tar.gz` |
| `SH-DS-02` | CelebA image archive | Needed for black-box and image-level transfer lines | [CelebA official page](https://mmlab.ie.cuhk.edu.hk/projects/CelebA.html) | Usually manual agreement / mirror choice | ~1.3 GB | `<DIFFAUDIT_ROOT>/Download/shared/datasets/celeba/img_align_celeba.zip` |
| `SH-DS-03` | CelebA annotation files | Needed to rebuild splits / metadata when required | [CelebA official page](https://mmlab.ie.cuhk.edu.hk/projects/CelebA.html) | Usually manual agreement / mirror choice | small | `<DIFFAUDIT_ROOT>/Download/shared/datasets/celeba/` |
| `SH-WT-01` | Stable Diffusion v1.5 base model | Required by CLiD bridge and any SD1.5-based black-box work | [HF stable-diffusion-v1-5](https://huggingface.co/stable-diffusion-v1-5/stable-diffusion-v1-5) | Yes, gated / license | ~4-7 GB | `<DIFFAUDIT_ROOT>/Download/shared/weights/stable-diffusion-v1-5/` |
| `SH-WT-02` | OpenAI CLIP ViT-L/14 | Core encoder for CLiD-style scoring | [HF openai/clip-vit-large-patch14](https://huggingface.co/openai/clip-vit-large-patch14) | No | ~1.7 GB | `<DIFFAUDIT_ROOT>/Download/shared/weights/clip-vit-large-patch14/` |
| `SH-WT-03` | BLIP image captioning large | Useful for reconstruction-based black-box variants and caption-side probes | [HF Salesforce/blip-image-captioning-large](https://huggingface.co/Salesforce/blip-image-captioning-large) | No | ~2 GB | `<DIFFAUDIT_ROOT>/Download/shared/weights/blip-image-captioning-large/` |
| `SH-WT-04` | Google DDPM CIFAR-10 32 | Useful shared baseline checkpoint for diffusion experiments and compatibility checks | [HF google/ddpm-cifar10-32](https://huggingface.co/google/ddpm-cifar10-32) | No | ~300 MB | `<DIFFAUDIT_ROOT>/Download/shared/weights/google-ddpm-cifar10-32/` |
| `SH-WT-05` | OpenCLIP / LAION CLIP fallback | Optional fallback if CLiD or new black-box variants benefit from open-CLIP family tests | [HF laion/CLIP-ViT-L-14-laion2B-s32B-b82K](https://huggingface.co/laion/CLIP-ViT-L-14-laion2B-s32B-b82K) | No | ~1.7 GB | `<DIFFAUDIT_ROOT>/Download/shared/weights/openclip-vit-l-14/` |
| `SH-DS-04` | LSUN subset archive(s) | Optional for dataset dependence / transfer experiments later | [LSUN page](https://www.yf.io/p/lsun) | Mirror / manual | very large | `<DIFFAUDIT_ROOT>/Download/shared/datasets/lsun/` |
...
| `GB-PAP-04` | SIDe paper / supplement | Needed if we implement this line | arXiv / supplementary | no or manual | small | `<DIFFAUDIT_ROOT>/Download/gray-box/papers/side/` |
| `GB-PAP-05` | Structural memorization paper / supplement | Needed if we implement this line | arXiv / supplementary | no or manual | small | `<DIFFAUDIT_ROOT>/Download/gray-box/papers/structural-memorization/` |

### Already local, no urgent external download required

- `external/PIA/` already exists
- `external/SecMI/` already exists as the full upstream reference clone, and `third_party/secmi/` already exists as the minimal in-repo integration surface
...
- `<DIFFAUDIT_ROOT>/Download/gray-box/supplementary/defenses/`
- `<DIFFAUDIT_ROOT>/Download/white-box/supplementary/defenses/`
