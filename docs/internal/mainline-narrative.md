...
DiffAudit 正在构建一个面向扩散模型成员推断风险的分层审计框架：admitted 主讲面仍是黑盒 `recon`、灰盒 `PIA + stochastic-dropout`、白盒 `GSA + W-1`。在吸收 `GPT-5.4` 第一轮与第二轮 raw results 后，近端执行顺序已经从“继续找下一条 GPU 题”压缩成更严格的 evidence-gated 链：`06-g1a` 的 per-sample `H1/H2` 路线都已在真实 packet 上 miss，`H5` 只保留为 internal-only set-level governance fallback；`05-cross-box` 已在 enlarged `GSA + PIA` matched packet 上确认 stable low-FPR tail lift，并把 promoted candidate 收束到 `logistic_2feature`；第一版 bounded `H4 tail-gated cascade` 只给出 auxiliary-only cost-saver 读法；`04-H2` 已完成最小 `4 / 4` negative-but-useful follow-up，不能写成 execution-ready successor。`X-156 / X-157 / X-158 / X-159 / X-160 / X-161 / X-162 / X-163` 又把 `04-H3 selective / suspicion-gated all-steps routing` 推进到 fresh `64 / 64` GPU scout、fixed-budget attacker scout 和 post-GPU review：fixed-budget selective 匹配 full all-steps dropout 的低 FPR tail，但 gate-leak 与 oracle-route escape falsifier 阻断 promotion，因此只能写成 candidate-only quality / perturbation-exposure idea。`X-168` 已给 `01-black-box H2 strength-response` 一个 positive-but-bounded 首轮结果：`AUC = 0.928955 / ASR = 0.859375 / TPR@1%FPR = 0.218750 / TPR@0.1%FPR = 0.218750`。`X-170` 已说明 H1 response-cloud geometry 不能驱动 promotion，因为 low-FPR 聚合为 `0.0 / 0.0`；`X-171` frequency-filter ablation 支持 H2 不是 high-frequency-only；`X-172` 非重叠 `128 / 128` validation 通过；`X-175` CPU stress 通过；`X-176` 非重叠 `256 / 256` validation 也通过，raw H2 logistic 为 `AUC = 0.913940 / ASR = 0.851562 / TPR@1%FPR = 0.171875 / TPR@0.1%FPR = 0.062500`，`lowpass_0_5` secondary 为 `0.140625 / 0.050781`；`X-177` 已把 H2 冻结为 strong validated candidate。`X-178` 随后确认 same-packet admitted `recon` comparator 在 X176 上协议不兼容；`X-179` 确认 X176 的 simple reconstruction-distance sanity comparators 已经足够做同包 sanity check，但不是 admitted `recon`，所以不释放 GPU。当前仍拒绝 promotion / `recon` replacement / consumer handoff。当前 `active GPU question = none`、`next_gpu_candidate = none`。`X-152` 已确认 `I-A` 热路径仍保留四指标、bounded adaptive 与 provenance-blocked 边界。`Finding NeMo / I-B` 已经不再是 `zero-GPU hold`，而是一个带真实 admitted bounded packet 的 `actual bounded falsifier`，`SMP-LoRA / DP-LoRA` 继续停留在 `metric-split bounded exploration branch`。

2026-04-29 的补充事实是：`G1-A / X-90` 已经不再是 `TMIA-DM 512-sample gap`。`X-141 / X-142` 生成两轮 matched `TMIA-DM 512` 表面，并在同一 `512 / 512` PIA/TMIA-DM 身份合同上完成两次 positive-but-bounded tri-score review；seed-2 macro 指标达到 `AUC = 0.859043 / ASR = 0.786133 / TPR@1%FPR = 0.118164 / TPR@0.1%FPR = 0.023438`。这应写成 `two-seed internal auxiliary gray-box evidence aggregation`，不是 admitted gray-box headline 替换；`PIA` 仍是当前灰盒主讲线。同日后续 `X-145 / X-146 / X-148 / X-150` 把 `03-H1 activation-subspace` 做成真实 GPU-safe scout，但序列只能收口为 `negative but useful`：基础 top-delta selector 过拟合，validation-regularized selector holdout 仍只有 `AUC = 0.625`，same-block cross-layer stability gate 的 primary holdout 降到 `AUC = 0.5625`。`X-158 / X-159 / X-160 / X-161 / X-162 / X-163` 则给出并收紧一条新的防御候选事实：selective all-steps gating 在 fresh `64 / 64` packet 上低 FPR 匹配 full all-steps dropout，但只能保留为 candidate-only；fixed-budget scout 进一步证明 gate-leak 和 oracle-route escape 会阻断部署和 promotion。`X-165` 又证明现有三路 shared surfaces 的共识融合不能稳定改善低 FPR；`X-166` 已把边界冻结；`X-168 / X-171 / X-172 / X-175 / X-176 / X-177` 已把黑盒 strength-response 推进为 strong validated positive-but-bounded candidate surface；`X-178 / X-179` 已把 admitted comparator 与 immediate GPU acquisition 两条捷径都关掉。`X-180` 已关闭为 `positive reselection / GPU hold`；当前执行项是治理清理，治理完成后的下一条 CPU-first lane 是 `X-181 I-A / cross-box boundary maintenance`，而不是继续扩大 GPU、替换 `recon` 或改变系统消费 schema。
...
6. 当前默认不放行新的 GPU question；研究重点已从“多线平推”改成报告驱动的近端链：
   - `G1-A / X-90` = two-seed internal auxiliary positive，已通过 `X-143 / X-144` 同步为 internal-only，不替换 `PIA`，也不触发第三个同合同 seed
   - `03-H1 activation-subspace` = GPU-safe scout infrastructure exists, but `X-145 / X-146 / X-148 / X-150` all stay below promotion; next activation attempt needs a genuinely different observable rather than another mean-profile selector
   - `05-cross-box` = promoted late-fusion support 主线
   - `04-defense` = `H2` 最小 4/4 negative-but-useful 已收口；`H3` selective all-steps gating 是 candidate-only positive-hardening / GPU hold，不释放更大 H3 GPU
...
- 一条能被系统消费的研究结果读链：`Research -> Runtime-Server -> Platform`

因此，研究与工程是耦合的：实验结果不能只存在于临时日志里，而要能够进入 admitted 结果、被 `Runtime-Server` 查询、被平台解释，并保持跨文档口径一致。
...
4. 把 `03-white-box` 固定为 medium-horizon gap：
   - `activation-subspace fingerprint` 的 mean-profile route 已被三轮 scout 收口为 negative-but-useful
   - 再考虑 `risky-subspace pruning / targeted unlearning`
   - 这条线当前用于准备真正的 white-box distinct second family，不抢占 `05 -> 04` 的近端资源
5. 把 `01-black-box` 固定为 `recon admitted + H2 validated candidate + parked candidate pool`：
   - `recon` 仍是 admitted 黑盒主讲证据
   - `strength-response curve` 现在是 validated positive-but-bounded candidate surface，下一步只允许 comparator/adaptive contract freeze
   - `response-cloud geometry / micro-bag statistical audit` 仍是 parked candidate pool
   - 当前不改变 Runtime/Platform schema，也不把 H2 写成 `recon` 替代
6. 将 `06-g1a` 与 direct `G1-A / X-90` 区分开：
   - per-sample surrogate `H1/H2` 路线仍然是 miss：`H1` 的 `256` teacher-calibrated validation 只到 `Spearman = 0.748677 / Pearson = 0.790525 / AUC = 0.687477`，`H2 RMIA / BASE temporal LR` primary `late-window mean` 只有 `AUC = 0.644142 / TPR@1%FPR = 0.007812 / threshold_cv = 0.806137`
   - direct `TMIA-DM 512` blocker 已被 `X-141 / X-142` 真实补齐，`G1-A / X-90` 现在是 `two-seed internal auxiliary positive`
   - 这不是 admitted gray-box headline，也不是继续让 `06` 占主动执行槽位；当前已完成 internal-only 边界同步，且后续 `03-H1 activation-subspace` GPU scout 已收口为 negative-but-useful
