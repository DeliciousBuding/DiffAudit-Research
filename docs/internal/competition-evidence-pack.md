...
3. 当前最成熟的主讲闭环是灰盒 `PIA`。
4. `PIA` 的攻击信号可解释为 `epsilon-trajectory consistency`，当前防御原型 `stochastic-dropout` 能在同协议下稳定压低攻击指标；对外必须按 `AUC / ASR / TPR@1%FPR / TPR@0.1%FPR` 四指标一起复述，并注明 adaptive 只到 bounded repeated-query review。
...
- `PIA` 是比赛中的算法主讲线，但 strongest claim 仍必须写成：
  - `workspace-verified + bounded repeated-query adaptive-reviewed + paper-alignment blocked by checkpoint/source provenance`
...
- baseline：
  - `PIA GPU512 baseline`
  - `AUC / ASR / TPR@1%FPR / TPR@0.1%FPR = 0.841339 / 0.786133 / 0.058594 / 0.011719`
- defended：
  - `PIA GPU512 + provisional G-1 stochastic-dropout(all_steps)`
  - `AUC / ASR / TPR@1%FPR / TPR@0.1%FPR = 0.828075 / 0.767578 / 0.052734 / 0.009766`

当前最重要的讲法不是“绝对数值多高”，而是：

- 主结果来自真实资产 mainline，而不是 smoke
- `all_steps` defended line 在 repeated-query adaptive review 后仍保持同向下降
- 灰盒口径不能只报 `AUC / ASR`，必须一起带 low-FPR 两档
- 当前结果仍只到 bounded repeated-query adaptive review，不能写成 validated privacy protection
...
- `Finding NeMo`
  - 当前已不再只是 `adapter-complete zero-GPU hold`
  - 当前最强诚实口径是：
    - one actual bounded admitted fixed-mask packet exists
    - current packet verdict = `negative but useful`
  - 不能写成 execution-ready、benchmark-ready、defense-positive 或已获后续 GPU release
- `SecMI`
  - 当前不应写成比赛主讲主线
  - 但也不能再写成 `blocked baseline`
  - 当前最诚实口径是：同一 `CIFAR-10` 资产线上的 `independent corroboration line`
- `TMIA-DM`
  - 当前不应写成比赛主讲主线
  - 当前最诚实口径是：`strongest packaged gray-box challenger`
  - `PIA vs TMIA-DM confidence-gated switching` 首轮 packet 已落地，但 verdict 是 `negative but useful`
- `variation real API`
  - 真实 API 资产仍 blocked
...
- `Research/workspaces/implementation/artifacts/unified-attack-defense-table.json`
  - 当前比赛主结果统一表
  - `Runtime-Server` / `Platform` 应优先从这里取黑盒、灰盒、白盒 headline 指标
...
- `PIA = workspace-verified + bounded repeated-query adaptive-reviewed + paper-alignment blocked by checkpoint/source provenance`
...
- 本项目当前最强灰盒主张是 `workspace-verified + bounded repeated-query adaptive-reviewed`，并同时显式携带 `paper-alignment blocked by checkpoint/source provenance`。
...
4. `Finding NeMo / SecMI / TMIA-DM` 不应被一起读成同一种状态：
   - `Finding NeMo = actual bounded falsifier, but no same-family GPU reopen`
   - `SecMI = same-asset independent corroboration line, not blocked baseline`
   - `TMIA-DM = strongest packaged gray-box challenger, but not promoted scorer family`
   现在放新的白盒同家族 GPU 只会越过既定 gate。
...
- `git status`：
  - `<DIFFAUDIT_ROOT>` 根目录本身不是 git worktree，`git status --short` 会在根目录返回 `fatal: not a git repository`
- `GPU`：
  - `nvidia-smi` 显示 `RTX 4070 Laptop GPU` 当前约 `3186MiB / 8188MiB`
  - 进程列表仅见 `Codex.exe / Claude.exe / Edge / Explorer` 等 `C+G` 进程
  - 没有可见 CUDA 训练/评估进程
- `当前 run`：
  - 进程命令行中未发现指向 `<DIFFAUDIT_ROOT>/Research` 的训练脚本
  - 最新与 GPU 决策相关的有效指标来自 `2026-04-13` 的 `SMP-LoRA T06` 记录；原始输出目录属于本地 scratch，不作为 Git evidence anchor
...
- legacy local scratch metric: `SMP-LoRA T06 AdamW low-lr`
  - `AUC = 0.5923295454545454`
  - `Accuracy = 0.5789473684210527`
- legacy local scratch metric: `SMP-LoRA T06 SGD matched-lr`
  - `AUC = 0.42113095238095233`
  - `Accuracy = 0.3684210526315789`

The durable decision anchor is the workspace intake packet above. The
raw local output paths are intentionally omitted because `Research/outputs/` is
ignored scratch, not portable repository evidence.
