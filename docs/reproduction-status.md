...
12. `W-1 / DPDM` 当前也不再是“仓库有了但还没接”。它已经有六类 comparator 证据；最新的 `strong-v2 max512` 结果 `auc=0.537201` 与 `max128` 的 `0.541199` 接近，说明当前白盒防御趋势在更大样本规模下仍能站住。
13. 当前 `DPDM` 的真正技术限制已从“环境阻塞”变成“benchmark 口径仍偏 runtime-smoke”。也就是说，白盒防御下一步应当在 stronger checkpoints 已存在、`max512` 已验证趋势稳定的前提下，继续向完整三 shadow 对齐，而不是继续证明 checkpoint 可以被读取。
...
5. 白盒攻击侧 `GSA` 已经站稳，下一步优先从 [whitebox-attack-defense-table.md](../workspaces/white-box/2026-04-08-whitebox-attack-defense-table.md) 出发，把 `W-1 strong-v2 max512` 接入统一总表，并吃完 `shadow-01 strong-v2` 后推进三 shadow comparator。

更新时间：`2026-04-08 08:12:00 +08:00`
