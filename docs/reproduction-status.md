...
2. `recon` 已经有统一 mainline smoke，并且现在补上了 dataset/checkpoint 直达 score artifact 的 runtime mainline 入口；`Stable Diffusion + DDIM` 已经从 `1-sample` 提升到 `25-sample public runtime-mainline`，`kandinsky_v22` 的最小真实 runtime-mainline 与 artifact-mainline 也已经产出 summary。
...
5. 下一步优先级建议：
   - 先把 `recon` 的公开资产从 `25-sample` 再扩到更大样本规模，并核准 target/shadow/member/non-member 映射
