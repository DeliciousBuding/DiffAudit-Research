...
12. `W-1 / DPDM` 当前也不再是“仓库有了但还没接”。它已经有五类 comparator 证据；最新的 defended-target + defended-shadows `strong-v2` 结果虽高于 smoke 版，但 `auc=0.541199` 仍明显低于 `GSA` 的 `0.97514`，方向上继续支持白盒防御有效。
13. 当前 `DPDM` 的真正技术限制已从“环境阻塞”变成“benchmark 口径仍偏 runtime-smoke”。也就是说，白盒防御下一步应当在 stronger checkpoints 已存在的前提下，继续扩大 comparator 评估规模并提高 benchmark 对齐度，而不是继续证明 checkpoint 可以被读取。
...
5. 白盒攻击侧 `GSA` 已经站稳，下一步优先从 [whitebox-attack-defense-table.md](../workspaces/white-box/2026-04-08-whitebox-attack-defense-table.md) 出发，把 `W-1 strong-v2` 接入统一总表，并决定是继续扩大 comparator 规模，还是再提升 defended 训练强度。

更新时间：`2026-04-08 07:40:00 +08:00`
