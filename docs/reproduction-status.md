...
| `variation` | `2024-arxiv-towards-black-box-membership-inference-diffusion-models.pdf` 对应 API-only 路线 | `code-ready + evidence-ready` | `plan-variation` / `probe-variation-assets` / `dry-run-variation` / `run-variation-synth-smoke` | [variation-synth-smoke](../experiments/variation-synth-smoke/summary.json), [variation-synth-smoke-local-20260408](../experiments/variation-synth-smoke-local-20260408/summary.json) | 本地 CPU synthetic smoke 已再次验证，说明 `Towards/variation` 可作为第二黑盒候选线保留；但仍缺真实 API 凭据、调用预算和真实 query image 集，不能写成真实 black-box mainline |
...
5. 白盒攻击侧 `GSA` 已经站稳，下一步优先从 [whitebox-attack-defense-table.md](../workspaces/white-box/2026-04-08-whitebox-attack-defense-table.md) 出发，把 `W-1 strong-v2 3-shadow full-scale` 接入统一总表，并启动下一档更强 defended training rung。

更新时间：`2026-04-08 09:24:00 +08:00`
