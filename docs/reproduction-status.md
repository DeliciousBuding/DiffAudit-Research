...
| `recon` | `2025-ndss-black-box-membership-inference-fine-tuned-diffusion-models.pdf` 与 `external/Reconstruction-based-Attack` | `code-ready + evidence-ready` | `plan-recon` / `probe-recon-assets` / `dry-run-recon` / `run-recon-eval-smoke` | [recon-eval-smoke](/D:/Code/DiffAudit/Project/experiments/recon-eval-smoke/summary.json) | 缺真实 LoRA checkpoint、生成图像目录、embedding 输入和目标/影子分割资产 |
...
3. 纯黑盒主线还没有真实 benchmark-ready 结果，因为 `recon` 和 `variation` 还停在 synthetic smoke 或 dry-run 级别。
4. 下一步优先级建议：
   - 先给 `recon` 接真实 artifact summary
