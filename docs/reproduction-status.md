...
2. `recon` 已经有统一 mainline smoke，并且现在补上了 dataset/checkpoint 直达 score artifact 的 runtime mainline 入口；`Stable Diffusion + DDIM` 已经从 `1-sample` 提升到 `50-sample public runtime-mainline`，`kandinsky_v22` 的最小真实 runtime-mainline 与 artifact-mainline 也已经产出 summary。
...
5. 下一步优先级建议：
   - 先把 `recon` 的公开资产从 `50-sample` 再扩到更大样本规模，并核准 target/shadow/member/non-member 映射
   - 再把 `DiT` 从 `step10 sample-smoke` 往更强的本地 checkpoint 采样证据推进，然后再决定是推进 `variation` 真实 API 调用还是补 `clid` 的真实资产驱动执行
6. 研究仓库现在已经新增本地 HTTP API 服务，入口见 [local-api.md](/D:/Code/DiffAudit/Project/docs/local-api.md)。当前已打通 `health`、模型目录、`recon` 最佳证据查询、workspace summary 查询，以及 `recon_artifact_mainline` / `recon_runtime_mainline` 的受控 job 提交与状态查询，平台联调应优先对接这条本地服务，而不是继续消费平台仓库内的 stub job 实现。
