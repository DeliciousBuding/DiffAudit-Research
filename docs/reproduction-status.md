...
12. `W-1 / DPDM` 当前也不再是“仓库有了但还没接”。它已经有八类 comparator 证据；最新的 three-shadow `strong-v2 full-scale` 结果 `auc=0.490813` 比 `3-shadow max512` 的 `0.462799` 略高，但仍显著低于 `GSA` 的 `0.97514`，说明当前白盒防御在更大评估规模下仍然能站住。
13. 当前 `DPDM` 的真正技术限制已从“环境阻塞”变成“benchmark 口径仍偏 runtime-smoke”。也就是说，白盒防御下一步应当从“继续扩大同一 checkpoint 的评估规模”转向“继续抬 defended 训练强度”，而不是继续证明 checkpoint 可以被读取。
