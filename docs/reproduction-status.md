...
12. `W-1 / DPDM` 当前也不再是“仓库有了但还没接”。它已经有七类 comparator 证据；最新的 three-shadow `strong-v2 max512` 结果 `auc=0.462799` 进一步低于两 shadow `max512` 的 `0.537201`，说明当前白盒防御在更完整的 `1k-3shadow` 结构下仍然能站住。
13. 当前 `DPDM` 的真正技术限制已从“环境阻塞”变成“benchmark 口径仍偏 runtime-smoke”。也就是说，白盒防御下一步应当在完整三 shadow comparator 已站住的前提下，决定是继续扩大 benchmark 规模，还是继续抬 defended 训练强度，而不是继续证明 checkpoint 可以被读取。
...
5. 白盒攻击侧 `GSA` 已经站稳，下一步优先从 [whitebox-attack-defense-table.md](../workspaces/white-box/2026-04-08-whitebox-attack-defense-table.md) 出发，把 `W-1 strong-v2 3-shadow max512` 接入统一总表，并决定下一轮是继续扩大 benchmark 规模，还是继续提高 defended 训练强度。

更新时间：`2026-04-08 09:02:00 +08:00`
