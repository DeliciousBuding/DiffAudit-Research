...
3. `DiT` 官方采样路径不再只停留在 `2-step` 极小 smoke，当前还存在本地 checkpoint 驱动的 [dit-sample-step10](../experiments/dit-sample-step10/summary.json)。
4. 纯黑盒主线还没有真实 benchmark-ready 结果，因为 `recon` 当前仍是公开子集级证据，target/shadow/member/non-member 的论文级语义映射尚未最终核准；`DiT` 仍停在官方 `sample-smoke` 级执行，`variation` 仍停在 synthetic smoke、artifact summary、upstream smoke 或 dry-run 级别。
5. 下一步优先级建议：
