...
`<DIFFAUDIT_ROOT>` 是本地项目根目录，可以位于任意工作目录。`Download\` 不属于 git 仓库。新机器要么从项目资产镜像复制整个 `Download\`，要么按 [research-download-master-list.md](../assets-and-storage/research-download-master-list.md) 的 first-wave 顺序下载。不要把这些大文件放回 `Research\external\`。
...
7. `<DIFFAUDIT_ROOT>\Download\manifests\research-download-manifest.json` 是 workspace-local manifest；真正随仓库走的是 [research-download-master-list.md](../assets-and-storage/research-download-master-list.md) 和 [data-and-assets-handoff.md](../assets-and-storage/data-and-assets-handoff.md)。
