...
| --- | --- | --- | --- | --- |
| 黑盒 | runtime evidence expanding | 黑盒主线已推进到更高一档公开 evidence：`recon` 已打通 `Stable Diffusion + DDIM` 的 `50-sample public runtime-mainline`、`kandinsky_v22` 的最小真实 runtime-mainline，`DiT` 本地 checkpoint 驱动的 `step10 sample-smoke` 也已通。 | 公开资产仍未达到论文级 `target/shadow/member/non-member` 语义核准，`variation` 与 `CLiD` 也还没进入真实资产闭环。 | 继续扩大 `recon` 公开子集规模，并视资源窗口决定是否把 `DiT` 从 `step10 sample-smoke` 推到更强步数或更高分辨率证据。 |
...
当前实验主线仍然是黑盒方向中的微调扩散模型成员推断。仓库状态已经从“最小链路打通”推进到“公开中样本 evidence”。
...
1. `recon` 已经不是 smoke 级验证，而是公开中样本 evidence 级主线。
2. `Stable Diffusion + DDIM` 的 `public-50` 是当前最值得继续扩展的基准。
3. `kandinsky_v22` 和 `DiT` 已经有最小可验证证据，但还没有达到和 `SD/DDIM public-50` 同等级的主证据强度。
...
1. 继续把 `recon` 从 `public-50` 扩到更大公开样本规模，并记录成本与指标稳定性。
2. 核准公开资产的 `target/shadow/member/non-member` 语义映射。
3. 视 GPU 窗口决定是继续推进 `DiT` 更高步数证据，还是补 `variation` / `CLiD` 的真实资产闭环。
