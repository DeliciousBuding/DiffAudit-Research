...
| --- | --- | --- | --- | --- |
| 黑盒 | runtime evidence expanding | 黑盒主线已推进到公开包可达到的最大子集 evidence：`recon` 已打通 `Stable Diffusion + DDIM` 的 `100-sample public runtime-mainline`；同时 `public-50` 仍保持当前最佳 AUC。`kandinsky_v22` 的最小真实 runtime-mainline 与 `DiT` 本地 checkpoint 驱动的 `step10 sample-smoke` 也已通。 | 公开资产仍未达到论文级 `target/shadow/member/non-member` 语义核准，且 `public-50` 与 `public-100` 的指标差异尚待解释；`variation` 与 `CLiD` 也还没进入真实资产闭环。 | 先核准映射语义并解释 `public-50` / `public-100` 的指标变化，再继续补 `DiT step50` 或 `Kandinsky 10/10`。 |
...
3. `kandinsky_v22` 和 `DiT` 已经有最小可验证证据，但还没有达到和 `SD/DDIM public-50/public-100` 同等级的主证据强度。
...
2. 视 GPU 窗口优先推进 `DiT step50`，或把 `Kandinsky` 扩到 `10/10`。
