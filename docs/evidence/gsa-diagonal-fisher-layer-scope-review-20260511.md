...
Review status: `mixed-but-not-gpu-ready`; no GPU task is released. The
machine-readable artifact verdicts remain per-layer contract outcomes.
...
target pair, but it does not beat the `raw_grad_l2_sq` baseline and the packet
is still only `1` sample per split.
...
conda run -n diffaudit-research python -X utf8 scripts\review_gsa_diagonal_fisher_feasibility.py `
  --workspace <gitignored-run-workspace> `
...
| Layer | Artifact verdict | Shadow Fisher AUC | Target Fisher AUC | Target Fisher ASR | Target scalar AUC | Target raw-grad L2 squared AUC |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
...
- The transferred Fisher score ties `raw_grad_l2_sq` on the target pair.
...
- No held-out shadow-fold check has shown `2/3` fold improvement over
  baselines.
- In this table, AUC measures ranking after shadow-frozen orientation, while
  ASR is best threshold accuracy on the oriented scores. With one member and
  one nonmember, a perfectly inverted ranking can still have `ASR = 0.5`
  because a constant threshold predicts one of the two samples correctly.

## Next Action

Run at most one more CPU-only stability board for
`up_blocks.1.attentions.0.to_v` with a larger still-small cap such as
`2` or `4` samples per split. If it does not beat `raw_grad_l2_sq` under
