...
| Track | Status | Notes |
| --- | --- | --- |
| Black-box `recon` | `evidence-ready` | Strongest black-box method and the selected non-CLiD lane for product-consumable strengthening. Public data limits strict paper-aligned claims. New recon summaries now emit strict-tail metric fields; the next GPU packet is pending score-artifact restore or bounded rerun to record the admitted public-100 strict-tail value. See [non-clid-blackbox-reselection.md](non-clid-blackbox-reselection.md) and [recon-product-validation-contract.md](recon-product-validation-contract.md). |
