...
| Gray-box `SecMI` | `evidence-ready` | Strong full-split supporting reference. The `25k / 25k` GPU packet reaches `AUC = 0.885833` on the stat head and `AUC = 0.946286` on the NNS auxiliary head, with NNS `TPR@0.1%FPR = 0.114`. It is not admitted Platform/Runtime evidence because the current row lacks the same consumer-boundary, structured cost, and adaptive-review contract as admitted PIA. See [secmi-full-split-admission-boundary-review.md](secmi-full-split-admission-boundary-review.md). |
