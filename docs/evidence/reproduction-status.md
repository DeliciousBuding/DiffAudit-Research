...
| Black-box `CLiD` | `bridge-contract-ready` | Selected as the next bounded black-box lane after H2 SD/CelebA text-to-image transfer was protocol-blocked. Local bridge artifact contract is reusable; next step is score output schema and low-FPR gate. See [black-box-next-lane-selection.md](black-box-next-lane-selection.md) and [clid-bridge-contract.md](clid-bridge-contract.md). |
