...
| Gray-box `ReDiffuse` | `asset-ready` | Candidate baseline-alignment line. The collaborator 750k bundle and checkpoint are runnable, a 64/64 direct-distance compatibility packet exists, and the existing PIA 800k checkpoint is runtime-probe compatible. The 750k ResNet scoring-contract parity packet is negative, so this is not admitted evidence. See [rediffuse-collaborator-integration-report.md](rediffuse-collaborator-integration-report.md), [rediffuse-800k-runtime-probe.md](rediffuse-800k-runtime-probe.md), and [rediffuse-resnet-parity-packet.md](rediffuse-resnet-parity-packet.md). |
