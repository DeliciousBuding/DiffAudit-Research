...
| Gray-box `ReDiffuse` | `asset-ready` | Candidate baseline-alignment line. The collaborator 750k bundle and checkpoint are runnable, a 64/64 direct-distance compatibility packet exists, and the existing PIA 800k checkpoint is runtime-probe compatible. Scoring-contract parity is unresolved, so this is not admitted evidence. See [rediffuse-collaborator-integration-report.md](rediffuse-collaborator-integration-report.md) and [rediffuse-800k-runtime-probe.md](rediffuse-800k-runtime-probe.md). |
