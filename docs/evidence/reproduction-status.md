...
| Track | Status | Notes |
| --- | --- | --- |
| Black-box `recon` | `evidence-ready` | Strongest black-box method and the selected non-CLiD lane for product-consumable strengthening. Public data limits strict paper-aligned claims. The bounded public-100 step30 rerun validates nonzero `TPR@0.1%FPR = 0.2`, but product-row promotion is blocked by AUC/ASR metric-source mismatch. See [non-clid-blackbox-reselection.md](non-clid-blackbox-reselection.md), [recon-product-validation-contract.md](recon-product-validation-contract.md), and [recon-product-validation-result.md](recon-product-validation-result.md). |
