...
| Split contract | **Pass**。存在带哈希的精确 `.npz` 清单：CIFAR100 (25000/25000)、STL10 (50000/50000)、Tiny-IN (50000/50000)。 |
| Score coverage | **Fail**。无 score 包、无生成的响应、无 ROC CSV。 |
| Metric contract | **Fail**。没有这些特定 checkpoint 的已发布指标。 |
| Provenance | **Partial**。划分可哈希但 checkpoint 和 score 缺失。 |
| Surface delta | **Pass**。不同数据集是真正的面增量。 |

**最短闭合路径**：使用已发布的划分索引从头训练一个 DDPM on CIFAR-100，然后
对训练好的 checkpoint 运行 PIA 或 GSA。这需要 GPU 时间（估计：与现有
DDPM/CIFAR-10 训练成本相当），产生的是自训练 checkpoint 而非公开第三方
checkpoint。科学价值在于划分索引来自第三方论文（ReDiffuse），即使 checkpoint
是本地训练的。
