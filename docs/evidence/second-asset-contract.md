...
| Score coverage | **Fail**。无 score 包、无生成的响应、无 ROC CSV。 |
| Metric contract | **Fail**。没有这些特定 checkpoint 的已发布指标。 |
| Provenance | **Partial**。划分可哈希，STL-10 管线资源预检已通过，但 checkpoint 和 score 缺失。 |
| Surface delta | **Pass**。不同数据集是真正的面增量。 |

**最短闭合路径**：优先用已发布的 STL-10 划分索引跑一个带硬上限的
DDPM/STL-10 scout，而不是直接 full-paper 训练。2026-05-25 预检已确认
官方 `UNet` + `GaussianDiffusionTrainer` 可在本地 CUDA 环境运行，batch `64`
校准峰值 `4.419 GB`。若 bounded scout 能产生可 score 的 checkpoint/attack
packet，再决定是否扩大训练预算。CIFAR-100 仍是同路径备选，但当前首选 STL-10，
因为本地数据已就绪且预检完成。

**风险**：自训练 checkpoint 的证据力弱于公开第三方 checkpoint。训练配方必须有
文档记录且可复现，结果才能被独立验证。微训练校准不是 membership metric，不得写成
AUC/ASR 结果或第二资产闭合。
...
### 最可达路径：ReDiffuse DDPM on CIFAR-100

ReDiffuse OpenReview 补充材料提供：
- CIFAR-100 (25000/25000) 的精确划分清单，附 SHA-256 哈希
- DDPM 的攻击/训练代码
- 有文档记录的训练配方

闭合缺口所需步骤：
1. 使用已发布的划分索引从头训练 DDPM on CIFAR-100（member =
   `mia_train_idxs`，来自清单）
2. 发布 checkpoint 及 SHA-256 哈希
3. 对 checkpoint 运行 PIA 或 GSA scoring
4. 发布 score 数组和四项头条指标

这是最短路径，因为：
- Split 合约已由第三方来源满足
- 攻击代码已与 workspace 兼容（PIA, GSA）
- 训练配方有文档记录
- CIFAR-100 足够小，GPU 预算有界

代价是从头训练 DDPM，GPU 工作量不小，但结果将是一个干净的第二面：相同架构
（DDPM），不同数据集（CIFAR-100），第三方划分溯源（ReDiffuse）。
