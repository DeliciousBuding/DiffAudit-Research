...
class DenoiseAttacker(DDIMAttacker):
    def ddim_reverse(self, x0, condition):
        intermediates = []
        # x = x0
        terminal_step = self.interval * self.attack_num
        ...
        for idx, step in enumerate(range(self.interval, terminal_step + self.interval, self.interval)):
            ...
            for i in range(self.average):
                ...
                for j in range(step, 0, -self.k):
                    ...
                    x_t_target = (x_t_target - sqrt_one_minus_alpha * predicted_noise) * sqrt_recip_alpha
...
class DenoiseSecMI(DDIMAttacker):
    def ddim_reverse(self, x0, condition):
        intermediates = []
        # x = x0
        terminal_step = self.interval * self.attack_num
    ...
    def ddim_denoise(self, x0, intermediates, condition):
        ...
        for step in range(0, terminal_step, self.interval):
            y_next = self.eps_getter(x0, condition, self.noise_level, step) * (self.get_p(step + self.interval) - self.get_p(step)) + self.get_y(x0, step)
            x0 = self.get_x(y_next, step + self.interval)
            

        x_t_target = x0


        for j in range(terminal_step, 0, -self.interval):
            alphas_t = self.noise_level[j]
            sqrt_recip_alpha = (1 / alphas_t).sqrt()
            sqrt_one_minus_alpha = (1 - alphas_t).sqrt()
        ...
        intermediates_denoise.append(x_t_target)

        return intermediates_denoise
    
class DenoisePIA(DDIMAttacker):
    def ddim_reverse(self, x0, condition):
        intermediates = []
        # x = x0
        terminal_step = self.interval * self.attack_num
    ...
    def ddim_denoise(self, x0, intermediates, condition):
        ...
        alphas_t_target = self.noise_level[terminal_step]

        epsilon = self.eps_getter(x0, condition, self.noise_level, 0)
