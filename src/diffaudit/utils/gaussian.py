...
def fit_univariate_gaussian(values: np.ndarray, *, min_variance: float = 1e-12) -> tuple[float, float]:
    ...
    variance = float(values_float.var())
...
def gaussian_log_likelihood_ratio(
    ...
    *,
    ...
    member_ll = -0.5 * (np.log(2.0 * np.pi * member_var) + ((values - member_mean) ** 2) / member_var)
    nonmember_ll = -0.5 * (np.log(2.0 * np.pi * nonmember_var) + ((values - nonmember_mean) ** 2) / nonmember_var)
