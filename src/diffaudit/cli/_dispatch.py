...
def _handle_secmi_pia_smokes(args: Any) -> int:
    ...
    if args.command == "run-midfreq-residual-sign-check":
        ...
        if args.sample_id_file is not None:
            ...
            if raw_text:
                if raw_text.startswith("["):
                    ...
                    sample_id_allowlist = [
                        int(line.strip()) for line in raw_text.splitlines() if line.strip()
                    ]
            else:
                sample_id_allowlist = []
...
def _handle_defenses(args: Any) -> int:
    ...
    if args.command == "prepare-defended-shadow-training-manifest":
        from diffaudit.defenses.risk_targeted_unlearning import (
            build_defended_shadow_training_manifest,
        )
    ...
    if args.command == "prepare-shadow-local-gsa-risk-preflight":
        from diffaudit.defenses.risk_targeted_unlearning import (
            build_shadow_local_gsa_risk_preflight,
        )

        shadow_ids = [value.strip() for value in str(args.shadow_ids).split(",") if value.strip()]
        payload = build_shadow_local_gsa_risk_preflight(
            workspace=args.workspace,
            gsa_loss_score_summary=args.gsa_loss_score_summary,
            shadow_ids=shadow_ids,
            top_k=args.top_k,
            provenance_status=args.provenance_status,
        )
        print(json.dumps(payload, indent=2, ensure_ascii=True))
        return 0 if payload["preflight_status"] == "complete" else 1
...
_COMMAND_HANDLERS: dict[str, CommandHandler] = {
    ...
    "run-midfreq-residual-sign-check": _handle_secmi_pia_smokes,
    ...
    "prepare-shadow-local-gsa-risk-preflight": _handle_defenses,
