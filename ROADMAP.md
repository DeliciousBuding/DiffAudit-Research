...
| CPU sidecar | none selected after completed Tracing the Roots feature-packet MIA replay (`AUC = 0.815826`, `TPR@1%FPR = 0.134000`) because the packet is positive but provenance-limited and does not expose raw target checkpoint/sample IDs or image query-response artifacts; CDI official code is a dataset-inference semantic shift with large ImageNet/COCO/model requirements and no ready small score packet; StyleMI is paper-only/style-mimicry-relevant but lacks public code, target checkpoint, exact artist/image split manifest, generated images, feature packets, and score files; cross-modal watch consumer boundary is synchronized and keeps SAMA/DLM plus VidLeaks/T2V out of Platform/Runtime rows; Fashion-MNIST clean-split loss, SimA score-norm, and score-Jacobian sensitivity are weak; VidLeaks is T2V related-method code-snapshot-only/live-repo-unavailable and lacks released target/video split/feature-score artifacts; SAMA is diffusion-language-model related-method code, not a current image/latent-image Lane A asset, and lacks released target/split/score artifacts; R125 DreamBooth forensics is a course-notebook with private GDrive target/query artifacts; SecMI-LDM is same-author support-family material with public dataset/checkpoint links but no independent second-asset role; Memorization-LDM is medical-LDM-relevant but request-gated and artifact-incomplete; MoFit is mechanism-relevant but code-TBW/artifact-incomplete; White-box GSA Zenodo is admitted-family provenance only; MIA_SD lacks public images/checkpoint/split/query-response; Quantile Regression is mechanism-reference but artifact-incomplete; MIAGM is code-reference-only and artifact-incomplete; Noise as a Probe is mechanism-relevant but reproduction-incomplete; Zenodo fine-tuned diffusion is paper-and-code-backed but split-manifest incomplete; LAION-mi remains metadata-only watch |
| Highest-value next action | Continue Lane A only with a non-duplicate candidate that exposes exact split artifacts, or reopen Tracing the Roots only if raw target checkpoint/sample manifests or regeneration assets appear; otherwise switch to a genuinely different runnable target contract or stop instead of writing another scope/audit chain |
| Stop condition | Do not expand Tracing the Roots into timestep, feature-family, seed, classifier, optimizer, or regularization matrices unless raw provenance/regeneration assets appear or a consumer-boundary decision explicitly allows feature-packet evidence; do not download CDI Google Drive model folders, ImageNet train/validation archives, MS-COCO 2014 train/validation archives, annotations, text embeddings, or submodule payloads unless a dataset-inference lane is explicitly opened with frozen manifests, checkpoint hashes, `P` size, p-value/low-FPR metric, and consumer boundary; do not train StyleMI-style LoRAs, scrape artist images, invent style/member splits, or build image-processing feature packets without public-safe target/split/response artifacts; do not expand Fashion-MNIST into score-Jacobian timestep, perturbation-scale, seed, scheduler, norm, or packet-size matrices; do not download WebVid-10M, MiraData, Panda-70M, T2V weights, generated videos, Gemini captions, or VBench outputs unless a T2V lane is explicitly opened; do not download gated language models, prepare MIMIR/NLP subsets, train DLM targets, or launch SAMA GPU jobs unless a text/DLM lane is explicitly opened; do not recreate the R125 DreamBooth private Colab/GDrive run, scrape report images into a pseudo-split, or treat six embedded MSE values as a DiffAudit packet; do not download SecMI-LDM SharePoint zips or package same-author SecMI support material as an independent second asset; do not request controlled memorization-LDM synthesized samples, download medical datasets, train target medical LDMs, or reconstruct that paper pipeline from scratch; do not implement MoFit surrogate/embedding optimization from scratch; do not download GSA `DDPM.zip` or replay admitted-family GSA as a new asset; do not scrape MIA_SD images or train SD1.5; do not train Quantile Regression STL10/Tiny-ImageNet targets or reconstruct SecMI splits from scratch; do not train MIAGM targets or regenerate distributions from scratch; do not implement Noise-as-Probe from scratch; do not download the full Zenodo fine-tuned diffusion archive, write another same-line Zenodo audit/scope note, build LoRA scoring, or reopen LAION-mi live URLs without manifest-backed split/query evidence |
...
### Sync 字段(2026-05-15 post-Diffusion-Memorization;覆盖下方 `Current Focus` 表格直至下一次三槽位变更)

| Field | 2026-05-15 value |
| --- | --- |
| Active work | Diffusion Memorization asset gate found a real `500`-row memorized-prompt manifest but closed it as memorization semantic-shift / large GDrive assets / no MIA release; ReDiffuse OpenReview supplement split-manifest audit found exact DDPM split indices but no checkpoint/score packet, so no CPU/GPU release; Tracing the Roots OpenReview supplement replay completed as executable feature-packet MIA evidence (`AUC = 0.815826`, `TPR@1%FPR = 0.134000`) but remains provenance-limited/no admitted promotion/no GPU release; CDI official artifact gate closed as code-public / dataset-inference semantic shift / large-assets-required after StyleMI paper-only asset gate; no active GPU question and no next GPU candidate; CommonCanvas pixel/CLIP/prompt/stability/denoising-loss weak; Fashion-MNIST PIA-loss, SimA score-norm, and score-Jacobian sensitivity scouts weak; MIDST TabDDPM nearest-neighbor and shadow-distributional scouts weak; Beans LoRA denoising-loss and parameter-delta sensitivity weak; LAION-mi fixed `25/25` URL probe failed; Zenodo fine-tuned diffusion is paper-and-code-backed but split-manifest incomplete; Noise as a Probe is mechanism-relevant but reproduction-incomplete; MIAGM is code-reference-only and artifact-incomplete; Quantile Regression is mechanism-reference but artifact-incomplete |
| Active GPU question | none selected after Diffusion Memorization semantic-shift gate, positive-but-feature-only Tracing the Roots replay, weak CommonCanvas pixel/CLIP/prompt/stability/denoising-loss, gradient-prototype, Fashion-MNIST PIA-loss/SimA score-norm/score-Jacobian sensitivity, MIDST TabDDPM, Beans LoRA denoising-loss/delta-sensitivity, LAION-mi URL-probe, StyleMI artifact-gate verdict, and CDI dataset-inference gate |
...
| CPU sidecar | none selected after completed Diffusion Memorization asset gate, ReDiffuse split-manifest audit, and Tracing the Roots feature-packet replay; Diffusion Memorization has a public `500`-row memorized-prompt manifest but no MIA split/response/score packet and a `2.60G` ground-truth archive; ReDiffuse official supplement has exact DDPM split indices but no target checkpoint/score packet, while Tracing the Roots has fixed `1000/1000` member/external train and eval feature tensors and a positive MIA metric but lacks raw target checkpoint/sample IDs and image query-response artifacts; none is admitted and none should be expanded into large downloads, training, feature/timestep, seed, or classifier matrices; CDI official code is a dataset-inference semantic shift with large ImageNet/COCO/model requirements and no ready small score packet; StyleMI is paper-only/style-mimicry-relevant but lacks public code, target checkpoint, exact artist/image split manifest, generated images, feature packets, and score files; cross-modal watch consumer boundary is synchronized; Fashion-MNIST clean-split loss, SimA score-norm, and score-Jacobian sensitivity are weak; VidLeaks is T2V related-method code-snapshot-only/live-repo-unavailable and lacks target T2V weights, exact video split manifests, generated videos, feature CSVs, and score packets; SAMA is DLM/NLP related-method code without released target DLM checkpoint, exact split manifests, or response/score packet and is out-of-scope for the current image-diffusion lane; R125 DreamBooth forensics lacks public LoRA checkpoint, exact split manifest, and query images despite six embedded MSE values; SecMI-LDM has public dataset/checkpoint links but repeats the SecMI support family and is not an independent second asset; Memorization-LDM lacks public target LDM checkpoint, exact split manifests, and generated response package; Quantile Regression lacks paper-specific code, exact target artifacts, and per-sample split manifests; MIAGM lacks target checkpoint/split/generated-distribution artifacts; Noise as a Probe lacks public code/split/checkpoint artifacts; Zenodo full download and further same-line audits are blocked on a public split manifest; and LAION-mi live URLs remain closed |
