...
Lane A 继续检查 `jinhaoduan/SecMI-LDM`。该 repo 不是 clean second asset:
公开 surface 是 HuggingFace Diffusers fork + 少量 SecMI LDM runner。root
README 是 stock Diffusers README;`scripts/secmi_ldm_pokemon.sh` 指向本地
`./checkpoints/sd-pokemon-checkpoint` 和 `./datasets`;`src/mia/secmi.py`
还 hardcode `/home/jd3734@drexel.edu/workspace/SecMI-LDM/checkpoints/sd-pokemon-checkpoint`。
Pokemon/COCO/LAION split 与图片也都是 local-only assumption,没有发布 hashable
target checkpoint、exact member/nonmember manifest 或 query/response package。
结论为 `diffusers-fork / support-family / artifact-incomplete / no download /
no GPU release`。当前 `active_gpu_question = none`,`next_gpu_candidate = none`,
`CPU sidecar = none selected`。见
...
| --- | --- | --- |
| SecMI-LDM asset verdict | support-family Diffusers fork with local-only Pokémon checkpoint, dataset, and split assumptions; not a clean Lane A second asset; no download or GPU release | [docs/evidence/secmi-ldm-asset-verdict-20260514.md](docs/evidence/secmi-ldm-asset-verdict-20260514.md) |
