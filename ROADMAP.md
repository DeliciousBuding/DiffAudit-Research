...
## 2026-05-15 Reconstruction-Based Attack Artifact Gate

Lane A intake checked `py85252876/Reconstruction-based-Attack` /
`Black-box Membership Inference Attacks against Fine-tuned Diffusion Models`
because it is a direct black-box fine-tuned diffusion membership candidate with
an open Zenodo package. The GitHub code at
`93ee8dd4d12697354cd182461a9aa268b8de63e6` exposes a reconstruction-distance
pipeline for LoRA-fine-tuned Stable Diffusion v1-5, generation, feature
extraction, and threshold/distribution/classifier attacks. The Zenodo record
`10.5281/zenodo.13371475` exposes a single
`extra_data-20240825T145405Z-001.zip` file (`736,366,195` bytes,
`md5:a52e197025c54c197b00674d398f2f6a`) described as some fine-tuned
checkpoints and datasets.

Range inspection of the ZIP central directory found `21` entries: four CelebA
LoRA checkpoint folders with `pytorch_lora_weights.bin`, `optimizer.bin`,
`scheduler.bin`, and `random_states_0.pkl`, plus dataset pickle files under
`partial-100-target/member`, `partial-100-target/non_member`,
`100-target/non_member`, and `100-shadow/non_member`. The checked ZIP central
directory does not expose generated responses, reconstruction-distance feature
packets, score rows, ROC arrays, metric JSON, ready verifier output, or the
complete four-way score-input set required by `test_accuracy.py`
(`target_member`, `target_non_member`, `shadow_member`, `shadow_non_member`).

Decision: `checkpoint-and-dataset-public / incomplete score-input split /
response-score artifacts missing / no full download / no GPU release / no
admitted row`. This candidate stays Research-only black-box watch-plus. Do not
download the Zenodo ZIP, Stable Diffusion v1-5 weights, or CelebA assets; do
not run LoRA inference, reconstruction-distance extraction, classifier attack
training, or a tiny GPU packet unless complete public score-input files,
generated response/feature packets, or a reviewed checkpoint/data-starter
response-generation lane appears. Current slots remain
`active_gpu_question = none`, `next_gpu_candidate = none`, and `CPU sidecar =
none selected after Reconstruction-Based Attack artifact gate`. See
[docs/evidence/reconstruction-based-attack-artifact-gate-20260515.md](docs/evidence/reconstruction-based-attack-artifact-gate-20260515.md).
...
| CPU sidecar | none selected after Reconstruction-Based Attack artifact gate. Do not download the Reconstruction-Based Attack Zenodo ZIP, SD v1-5 weights, or CelebA assets; do not run LoRA inference, reconstruction-distance extraction, classifier replay, or response generation. Keep the same no-download/no-GPU gates for TMIA-DM, Shake-to-Leak, FSECLab MIA-Diffusion, MT-MIA, LSA-Probe, DDMD, DIFFENCE, MIAHOLD/HOLD++, DMin, ELSA, Memorization Anisotropy, FERMI, DurMI, FMIA, CLiD, StablePrivateLoRA, MIDM, GGDM, Diffusion Memorization, ReDiffuse, and same-family MIDST expansions; do not promote any watch/candidate row without score artifacts and product-bridge handoff. |
...
| Field | Current value |
| --- | --- |
| Active work | `Reconstruction-Based Attack artifact gate completed; code plus Zenodo checkpoint/dataset package are public, but generated responses, reconstruction-distance features, score rows, ROC arrays, metric JSON, ready verifier, and a complete four-way score-input split are missing.` |
| Current GPU candidate | none selected |
| CPU sidecar | none selected after Reconstruction-Based Attack artifact gate. Do not download the Reconstruction-Based Attack Zenodo ZIP, SD v1-5 weights, or CelebA assets; do not run response generation, reconstruction-distance extraction, classifier replay, TMIA-DM temporal-noise reconstruction, Shake-to-Leak fine-tuning/data extraction, FSECLab training/sampling/attacks, or cross-modal dataset/checkpoint downloads. |
| Active GPU question | none after Reconstruction-Based Attack artifact gate, TMIA-DM temporal artifact gate, Shake-to-Leak code artifact gate, FSECLab MIA-Diffusion code artifact gate, MT-MIA relational diffusion score-packet gate, LSA-Probe music diffusion mock-data gate, DualMD / DistillMD defense artifact gate, DIFFENCE classifier-defense artifact gate, MIAHOLD/HOLD++ defense artifact gate, Quantile Diffusion MIA support-packet replay, DMin data attribution gate, ELSA Health Privacy Challenge gate, Memorization Anisotropy artifact gate, FERMI tabular artifact gate, DurMI TTS artifact gate, GenAI Confessions black-box artifact gate, SimA score-based artifact gate, FMIA OpenReview frequency artifact gate, CLiD identity-manifest gate, CLiD official inter-output replay, StablePrivateLoRA defense artifact gate, MIDM artifact gate, cross-modal watch consumer-boundary sync, GGDM Zenodo artifact gate, MIDST Blending++ official score-export scout, SecMI/PIA adaptive comparability board, Diffusion Memorization semantic-shift gate, positive-but-feature-only Tracing the Roots replay, weak CommonCanvas/Fashion-MNIST/Beans scouts, failed LAION-mi URL probe, StyleMI artifact-gate verdict, and CDI dataset-inference gate |
