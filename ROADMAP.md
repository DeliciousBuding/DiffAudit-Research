...
promotion`。当前 `active_gpu_question = none`,`next_gpu_candidate = none`,
...
### Sync 字段(2026-05-15 post-MIDST-EPT;覆盖下方 `Current Focus` 表格直至下一次三槽位变更)

| Field | 2026-05-15 value |
| --- | --- |
| Active work | MIDST TabDDPM EPT scout adapted a genuinely different MIA-EPT-style error-prediction profile to the local single-table benchmark, learned train shadow folders (`AUC = 0.851961`), but closed as weak transfer on dev+final (`AUC = 0.530089`, `TPR@1%FPR = 0.029500`); Diffusion Memorization asset gate found a real `500`-row memorized-prompt manifest but closed it as memorization semantic-shift / large GDrive assets / no MIA release; ReDiffuse OpenReview supplement split-manifest audit found exact DDPM split indices but no checkpoint/score packet, so no CPU/GPU release; Tracing the Roots OpenReview supplement replay completed as executable feature-packet MIA evidence (`AUC = 0.815826`, `TPR@1%FPR = 0.134000`) but remains provenance-limited/no admitted promotion/no GPU release; CDI official artifact gate closed as code-public / dataset-inference semantic shift / large-assets-required after StyleMI paper-only asset gate; no active GPU question and no next GPU candidate; CommonCanvas pixel/CLIP/prompt/stability/denoising-loss weak; Fashion-MNIST PIA-loss, SimA score-norm, and score-Jacobian sensitivity scouts weak; MIDST TabDDPM nearest-neighbor, shadow-distributional, and EPT scouts weak; Beans LoRA denoising-loss and parameter-delta sensitivity weak; LAION-mi fixed `25/25` URL probe failed; Zenodo fine-tuned diffusion is paper-and-code-backed but split-manifest incomplete; Noise as a Probe is mechanism-relevant but reproduction-incomplete; MIAGM is code-reference-only and artifact-incomplete; Quantile Regression is mechanism-reference but artifact-incomplete |
| Active GPU question | none selected after MIDST TabDDPM EPT weak transfer, Diffusion Memorization semantic-shift gate, positive-but-feature-only Tracing the Roots replay, weak CommonCanvas pixel/CLIP/prompt/stability/denoising-loss, gradient-prototype, Fashion-MNIST PIA-loss/SimA score-norm/score-Jacobian sensitivity, MIDST TabDDPM nearest-neighbor/shadow-distributional, Beans LoRA denoising-loss/delta-sensitivity, LAION-mi URL-probe, StyleMI artifact-gate verdict, and CDI dataset-inference gate |
