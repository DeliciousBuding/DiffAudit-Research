...
| --- | --- |
| Active work | Cross-modal watch consumer boundary synchronized after GGDM Zenodo artifact gate. SAMA/DLM, VidLeaks/T2V, and GGDM/graph-diffusion remain related-method watch only; no Platform row, Runtime schema change, admitted bundle row, product copy, or recommendation logic is released. GGDM itself is code-only and lacks fixed graph target checkpoint, exact member/nonmember graph manifest, generated graph cache, score/ROC artifact, metrics packet, and public AWE module. MIDST Blending++ remains the strongest MIDST result so far (`dev+final AUC = 0.598079`, `TPR@1%FPR = 0.095750`) but below the `0.60` reopen floor. |
| Active GPU question | none selected after cross-modal watch consumer-boundary sync, GGDM Zenodo artifact gate, MIDST Blending++ official score-export scout, SecMI/PIA adaptive comparability board, Diffusion Memorization semantic-shift gate, positive-but-feature-only Tracing the Roots replay, weak CommonCanvas/Fashion-MNIST/Beans scouts, LAION-mi URL probe, StyleMI artifact-gate verdict, and CDI dataset-inference gate |
...
| CPU sidecar | none selected after cross-modal watch consumer-boundary sync. Do not request the withheld AWE module, download graph datasets, train EDP-GNN/GDSS/DiGress targets, regenerate graph sample caches, add DLM/T2V/graph Platform rows or Runtime schemas, expand MIDST into XGBoost / Optuna retraining, Gower feature-matrix variants, TabSyn, multi-table MIDST, Platform/Runtime admitted rows, SecMI GPU, NNS promotion, or learned fusion/gating. |
