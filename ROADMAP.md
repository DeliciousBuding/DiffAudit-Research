...
| CPU sidecar | none selected after Discrete DLM withdrawn artifact gate. The arXiv `2605.16445` DLM check is a withdrawn paper-source-only watch verdict with no official code, no target/split artifacts, no score/response packet, no download, no CPU/GPU sidecar, and no Platform/Runtime impact. |
