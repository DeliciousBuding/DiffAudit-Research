...
### Sync 字段(2026-05-15 post-GGDM;覆盖下方 `Current Focus` 表格直至下一次三槽位变更)

| Field | 2026-05-15 value |
| --- | --- |
| Active work | MIDM artifact gate completed from public GitHub repo inspection. MIDM is a genuine image diffusion MIA candidate with FFHQ DDPM loss/likelihood attack code, `ffhq_1000_idx.npy` split semantics, `1000/1000` labels in `Example.ipynb`, and fixed-FPR TPR metric code, but it lacks committed member/nonmember manifests, ready HDF5 score packets, ROC/metric artifacts, notebook outputs, and accessible checkpoint metadata in this environment. |
| Active GPU question | none selected after MIDM artifact gate, cross-modal watch consumer-boundary sync, GGDM Zenodo artifact gate, MIDST Blending++ official score-export scout, SecMI/PIA adaptive comparability board, Diffusion Memorization semantic-shift gate, positive-but-feature-only Tracing the Roots replay, weak CommonCanvas/Fashion-MNIST/Beans scouts, LAION-mi URL probe, StyleMI artifact-gate verdict, and CDI dataset-inference gate |
...
| CPU sidecar | none selected after MIDM artifact gate. Do not download FFHQ thumbnails, request or scrape MIDM checkpoint access, train MIDM DDPM, run MIDM loss/likelihood scoring from scratch, request the withheld AWE module, download graph datasets, train EDP-GNN/GDSS/DiGress targets, regenerate graph sample caches, add DLM/T2V/graph Platform rows or Runtime schemas, expand MIDST into XGBoost / Optuna retraining, Gower feature-matrix variants, TabSyn, multi-table MIDST, Platform/Runtime admitted rows, SecMI GPU, NNS promotion, or learned fusion/gating. |
...
| CPU sidecar | none selected after MIDM artifact gate. Do not download FFHQ thumbnails, request or scrape MIDM checkpoint access, train MIDM DDPM, run MIDM loss/likelihood scoring from scratch, request withheld AWE code, download graph datasets, train EDP-GNN/GDSS/DiGress targets, regenerate graph sample caches, add DLM/T2V/graph Platform rows or Runtime schemas, or expand MIDST Blending++ into retraining / Gower / TabSyn / multi-table / admitted rows. |
| Active GPU question | none after MIDM artifact gate, cross-modal watch consumer-boundary sync, GGDM Zenodo artifact gate, MIDST Blending++ official score-export scout, SecMI/PIA adaptive comparability board, Diffusion Memorization semantic-shift gate, positive-but-feature-only Tracing the Roots replay, weak CommonCanvas/Fashion-MNIST/Beans scouts, failed LAION-mi URL probe, StyleMI artifact-gate verdict, and CDI dataset-inference gate |
