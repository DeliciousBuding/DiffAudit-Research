...
## 2026-05-25 ReDiffuse STL-10 bounded scout 结果

最新决策：ReDiffuse DDPM/STL-10 的唯一有界 scout 已完成，产出了 checkpoint
和 `512` 行 score packet，但固定 timestep denoising-loss 成员信号随机级，不能释放
下一轮 GPU 扩参，也不能作为第二资产或 Platform/Runtime 准入结果。
...
指标：`AUC = 0.4996337890625`，`ASR = 0.509765625`，
`TPR@1%FPR = 0.01171875`，`TPR@0.1%FPR = 0.0`，非成员分母 `256`，
最小非零 FPR `0.00390625`。这说明管线可执行、score packet 可生成，但该短程
target + 固定 timestep denoising-loss 不携带可用成员信号。

因此当前 slots 收回为：
`active_gpu_question = none`，`next_gpu_candidate = none`，
`CPU sidecar = none selected after ReDiffuse STL-10 bounded scout weak result`。

不要把这个弱结果扩成 step-count、seed、timestep、batch-size、subset-size、EMA、
scheduler 或 denoising-loss 矩阵。只有出现公开第三方 STL-10 checkpoint/score packet、
真正不同的成员 observable，或明确批准长训预算并提前定义 checkpoint/score 发布合约时，
才重新开启 ReDiffuse STL-10。See
...
pipeline，不围绕该包新增 CLI/validator/长文档。该审计不改变 2026-05-25
feature-packet 消费边界修正后的 none slots。
See
...
| CPU sidecar | none selected after ReDiffuse STL-10 bounded scout weak result. The only released scout completed `300` steps and produced a `256 / 256` score packet, but fixed-timestep denoising-loss was random-level (`AUC = 0.4996337890625`, `ASR = 0.509765625`, `TPR@1%FPR = 0.01171875`, `TPR@0.1%FPR = 0.0`). |
