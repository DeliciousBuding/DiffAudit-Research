...
| CPU sidecar | none selected after IAR privacy artifact gate. arXiv `2502.02514` / official `sprintml/privacy_attacks_against_iars` is code-public and reports strong image-autoregressive privacy results, but it has no committed score/ROC/metric/verifier packet, requires ImageNet and model downloads, and is outside the current diffusion/latent-image admitted consumer family. |
| Latest mechanism watch | IAR Privacy Attacks / arXiv `2502.02514` is image-generation privacy watch-plus with official code and strong reported MIA/DI/extraction claims, but it targets image autoregressive models rather than current diffusion/latent-image rows. Reopen only if compact public score/ROC/metric packets, immutable ImageNet row manifests, model hashes, generated sample packets, and a no-training verifier appear, or if DiffAudit explicitly opens an image-autoregressive consumer boundary. |
...
| Latest closed search branch | IAR Privacy Attacks / arXiv `2502.02514` is closed as image-autoregressive code-public watch-plus unless row-bound public replay artifacts appear or a reviewed IAR consumer boundary is opened. Silent Brush / Art Arena is closed as style-leakage semantic-shift evidence unless a reviewed style-leakage consumer boundary or row-bound membership artifacts appear. Trajectory generation privacy / arXiv `2605.15246` is closed as cross-domain paper-source-only evidence unless official code, immutable trajectory target/split manifests, generated trajectory packets, reusable score or ROC/metric artifacts, a verifier, and a reviewed trajectory consumer boundary appear. Model Will Tell / arXiv `2403.08487` is closed as DRC paper-source-only evidence unless official code, immutable Cifar/CelebA/FFHQ target/split manifests, restored response packets, reusable score or ROC/metric artifacts, and a verifier appear. Discrete DLM / arXiv `2605.16445` is closed as withdrawn paper-source-only evidence unless a current non-withdrawn version plus official public code, immutable target/split manifests, reusable score or response packets, ROC/metric artifacts, and a reviewed text/DLM consumer boundary appear. The GitHub lightweight diffusion MIA triage is closed as false-positive evidence unless one of the checked repositories publishes checkpoint-bound splits plus reusable score/response and metric artifacts. The bounded public CopyMark `laion_mi` binding gate is also closed unless authors publish a compact row-binding manifest, restore a public identifier column, or release another public file that binds the official score artifacts back to exact public member rows. The collaborator Stable Diffusion ReDiffuse bundle remains candidate-only and does not reopen this public-asset gate. |
