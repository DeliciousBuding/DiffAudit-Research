...
| --- | --- |
| Active work | GGDM Zenodo artifact gate completed from Zenodo `17946102`. It is a graph-diffusion cross-modal related-method watch item, but the small code archive lacks a fixed graph target checkpoint, exact member/nonmember graph manifest, generated graph cache, score/ROC artifact, metrics packet, and public AWE module; verdict is `graph-diffusion cross-modal watch / code-only artifact / no score packet / no GPU release`. MIDST Blending++ remains the strongest MIDST result so far (`dev+final AUC = 0.598079`, `TPR@1%FPR = 0.095750`) but below the `0.60` reopen floor. SecMI/PIA remains support-only; Diffusion Memorization, ReDiffuse, Tracing Roots, CDI, CommonCanvas, Fashion-MNIST, Beans, LAION-mi, Zenodo, Noise as a Probe, MIAGM, and Quantile Regression remain closed/hold unless a new manifest-backed target/split/response or score packet appears. |
| Active GPU question | none selected after GGDM Zenodo artifact gate, MIDST Blending++ official score-export scout, SecMI/PIA adaptive comparability board, Diffusion Memorization semantic-shift gate, positive-but-feature-only Tracing the Roots replay, weak CommonCanvas/Fashion-MNIST/Beans scouts, LAION-mi URL probe, StyleMI artifact-gate verdict, and CDI dataset-inference gate |
...
| CPU sidecar | none selected after GGDM Zenodo artifact gate. Do not request the withheld AWE module, download graph datasets, train EDP-GNN/GDSS/DiGress targets, regenerate graph sample caches, expand MIDST into XGBoost / Optuna retraining, Gower feature-matrix variants, TabSyn, multi-table MIDST, Platform/Runtime admitted rows, SecMI GPU, NNS promotion, or learned fusion/gating. |
...
| Field | Current value |
| --- | --- |
| Active work | `GGDM Zenodo artifact gate completed from Zenodo 17946102. It is graph-diffusion cross-modal watch with a small code archive, but no fixed graph target checkpoint, exact member/nonmember graph manifest, generated graph cache, score/ROC artifact, metrics packet, or public AWE module; verdict is graph-diffusion cross-modal watch / code-only artifact / no score packet / no GPU release. MIDST Blending++ remains the strongest MIDST result so far (dev+final AUC = 0.598079, TPR@1%FPR = 0.095750) but below the 0.60 AUC reopen floor. SecMI/PIA remains support-only; prior Diffusion Memorization, ReDiffuse, Tracing Roots, CDI, CommonCanvas, Fashion-MNIST, Beans, LAION-mi, Zenodo, Noise as a Probe, MIAGM, and Quantile Regression remain closed/hold unless a new manifest-backed target/split/response or score packet appears.` |
| Current GPU candidate | none selected |
| CPU sidecar | none selected after GGDM Zenodo artifact gate. Do not request withheld AWE code, download graph datasets, train EDP-GNN/GDSS/DiGress targets, regenerate graph sample caches, or expand MIDST Blending++ into retraining / Gower / TabSyn / multi-table / admitted rows. |
| Active GPU question | none after GGDM Zenodo artifact gate, MIDST Blending++ official score-export scout, SecMI/PIA adaptive comparability board, Diffusion Memorization semantic-shift gate, positive-but-feature-only Tracing the Roots replay, weak CommonCanvas/Fashion-MNIST/Beans scouts, failed LAION-mi URL probe, StyleMI artifact-gate verdict, and CDI dataset-inference gate |
