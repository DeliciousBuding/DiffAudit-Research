...
| Field | Value |
| --- | --- |
| Active GPU question | ReDiffuse DDPM/STL-10 bounded scout |
| Next GPU candidate | One bounded STL-10 DDPM pipeline scout only; no full-paper training, no `800k` step run, no Tiny-ImageNet or Stable Diffusion download. |
| CPU sidecar | none; ReDiffuse STL-10 split/statistics/resource preflight is complete. The `50k / 50k` split binds to STL-10 unlabeled rows, low-level image-statistics probe is random on holdout (`AUC = 0.4994776215625`), and official DDPM `UNet` + `GaussianDiffusionTrainer` batch `64` calibration fits local CUDA (`4.419 GB` peak). |
