...
### 历史决策状态（已被 2026-05-25 修正覆盖）

本节保留 2026-05-23 扫描结论和候选排序，但不再作为当前执行命令。
当时提出的 ReDiffuse STL-10 补充材料下载、划分提取、CPU 侧预检和后续有界
GPU 训练已被 2026-05-25 Stable Diffusion ReDiffuse source-label 边界审计及
Feature-Packet 消费边界修正覆盖：ReDiffuse OpenReview split manifests 仍是清晰的
第二资产候选线索，但公开面缺 target checkpoint、response/feature cache、
score packet、ROC CSV 和 metric artifact，因此当前不请求数据、不下载权重、
不启动 CPU 预检或 GPU 训练。
...
**当前执行口径**：以 2026-05-25 Feature-Packet 消费边界修正段的最新 none slots 为准。
