...
- gray-box main evidence: `PIA GPU512 adaptive-reviewed baseline + provisional G-1 (all_steps)`；当前最强 challenger: `TMIA-DM late-window long_window`
...
1. 灰盒当前优先不是再证明 `TMIA-DM` 存在，而是把 `PIA headline + TMIA challenger` 这一结构稳定传递给更高层。
2. 下一条灰盒研究任务应在 `TMIA-DM` 新 defense hypothesis 与材料化总表之间择优，而不是回退到 naive fusion。
