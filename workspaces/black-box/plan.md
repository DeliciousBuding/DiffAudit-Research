...
- `CLiD`: next selected bounded lane; local bridge preparation succeeded, and
  bridge contract validation passed. Score-schema gate design is the next CPU
  step.
...
Freeze the CLiD score output schema and low-FPR gate before any GPU packet.
...
prompt-conditioned black-box lane, with bridge contract validation complete.
