...
- `CLiD`: next selected bounded lane; local bridge preparation succeeded, and
  bridge contract validation passed. Score-summary gate is defined; tiny
  score-bridge planning is the next CPU step.
...
Map the prepared CLiD bridge outputs into the frozen score-summary schema before
any GPU packet. Keep status synchronized with
...
prompt-conditioned black-box lane, with bridge and score schema gates complete.
