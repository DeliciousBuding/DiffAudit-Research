...
```powershell
conda run -n diffaudit-research python <DIFFAUDIT_ROOT>/Research/external/DPDM/main.py `
  --mode train `
  --workdir runs/dpdm-cifar10-32-eps10-gpu-smoke-v7 `
  --config <DIFFAUDIT_ROOT>/Research/external/DPDM/configs/cifar10_32/train_eps_10.0.yaml `
  --root_folder <DIFFAUDIT_ROOT>/Research/external/DPDM `
  -- setup.n_gpus_per_node=1 `
     setup.omp_n_threads=8 `
     setup.backend=gloo `
     setup.master_address=127.0.0.1 `
     setup.master_port=6028 `
     data.path=<DIFFAUDIT_ROOT>/Research/external/DPDM/data/processed/cifar10.zip `
