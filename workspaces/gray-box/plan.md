...
- `next step`: 保持 `stochastic-dropout = provisional G-1 (repeat-confirmed at GPU512)`；把 `SecMI` 固定为独立 corroboration line 而不是 blocked baseline；不再为 naive `PIA + SecMI` fusion 消耗预算；`SimA` 当前不放 GPU；`TMIA-DM` 的 `late_steps_only` dropout ablation 已确认弱于 `all_steps`，当前最短下一步应转向新的 defense hypothesis 或材料化总表，而不是继续追 `late_steps_only`、重开 `short_window`/naive fusion，或直接跳到 `MoFit`
...
12. 当前 `TMIA-DM` 已完成最小 `protocol probe`、一次 repeat、same-split 比较、late-window refine、两次正向 `GPU128` rung，以及两次正向 `GPU256` rung；结论是该线已成为稳定 GPU challenger，且 `late_steps_only` defense ablation 也已证伪为弱解，下一条任务应是材料化总表或新的 defense hypothesis
