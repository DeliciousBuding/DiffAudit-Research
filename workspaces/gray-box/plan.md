...
- `next step`: 保持 `stochastic-dropout = provisional G-1 (repeat-confirmed at GPU512)`；把 `SecMI` 固定为独立 corroboration line 而不是 blocked baseline；不再为 naive `PIA + SecMI` fusion 消耗预算；`SimA` 当前不放 GPU；灰盒下一步转到 `TMIA-DM protocol / asset decomposition`，而不是立即 `SimA` reopen 或直接跳到 `MoFit`
